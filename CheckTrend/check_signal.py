"""
Bot Check Signal - 5 Bước Xác Định Long/Short (V2)
Dựa trên tài liệu: 5_buoc_xac_dinh_long_short_BTC_v2.md

Chạy mỗi 5 phút, chỉ gửi Telegram khi có tín hiệu BUY hoặc SELL

B1: Xác định Trendline (swing high/low) - Rule cứng: phá trendline → mất hiệu lực
B2: Xác định Mô hình giá (Tiếp diễn vs Đảo chiều) - Đảo chiều ngược trend → độ tin cậy thấp
B3: Kẻ Fibonacci (0.382-0.618 cho entry, ưu tiên 0.5-0.618) - Rule cứng: TP phải đúng phía
B4: Vẽ Hỗ trợ/Kháng cự (Supply/Demand zones) - Mạnh nhất khi trùng Fibo/trendline
B5: Tổng hợp quyết định Long/Short (Checklist A+)
"""
...
def main():
    print(f"\n{'='*70}")
    print(f"📊 BOT CHECK SIGNAL - 5 BƯỚC XÁC ĐỊNH LONG/SHORT (V2)")
    print(f"⏰ Chạy mỗi 5 phút - Chỉ gửi Telegram khi có tín hiệu BUY/SELL")
    print(f"{'='*70}\n")
    ...
    # Lưu trạng thái signal trước đó để tránh spam (chỉ gửi khi thay đổi)
    previous_signals = {}  # {symbol: (signal, confidence)}
    ...
    try:
        while True:
            loop_count += 1
            print(f"\n{'='*70}")
            print(f"🔄 Lần kiểm tra #{loop_count} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            print(f"{'='*70}\n")
            ...
            for symbol_base in SYMBOLS:
                ...
                if analysis:
                    ...
                    # Chỉ gửi Telegram khi có tín hiệu BUY hoặc SELL
                    if signal in ['BUY', 'SELL']:
                        # Kiểm tra xem signal có thay đổi không
                        prev_signal = previous_signals.get(symbol, (None, None))
                        signal_changed = (prev_signal[0] != signal) or (prev_signal[1] != confidence)
                        
                        if signal_changed or loop_count == 1:
                            # Gửi Telegram
                            telegram_msg = format_telegram_message(symbol, analysis)
                            print(f"\n📤 Đang gửi Telegram (Signal: {signal})...")
                            print(f"   Độ dài message: {len(telegram_msg)} ký tự")
                            
                            if send_telegram(telegram_msg):
                                print(f"✅ Đã gửi tín hiệu {signal} về Telegram")
                                # Lưu trạng thái
                                previous_signals[symbol] = (signal, confidence)
                            else:
                                print(f"❌ Không thể gửi Telegram sau 3 lần thử")
                        else:
                            print(f"⏭️ Signal {signal} ({confidence}) không thay đổi - Bỏ qua gửi Telegram")
                    else:
                        # NEUTRAL hoặc NO_TRADE - không gửi Telegram
                        print(f"⏭️ Signal: {signal} ({confidence}) - Không gửi Telegram")
                        if decision['reasons']:
                            print("Lý do:")
                            for reason in decision['reasons'][:3]:  # Chỉ hiển thị 3 lý do đầu
                                print(f"  {reason}")
                else:
                    print(f"⚠️ Không có dữ liệu cho {symbol_base}")
            
            # Đợi 5 phút (300 giây) trước lần kiểm tra tiếp theo
            print(f"\n{'='*70}")
            print(f"⏳ Đợi 5 phút trước lần kiểm tra tiếp theo...")
            print(f"{'='*70}\n")
