...
# Rule "VÀO LỆNH CHUẨN NHẤT" - 3 điều kiện cùng lúc
ENABLE_GOLDEN_ENTRY = True  # Bật/tắt rule "Vào lệnh chuẩn nhất"
EMA_FAST = 9   # EMA9 cho momentum
EMA_SLOW = 21  # EMA21 cho momentum
CANDLE_BODY_MIN_RATIO = 0.5  # Nến dài tối thiểu 50% toàn nến (momentum)
CANDLE_BODY_MAX_RATIO = 0.8  # Nến dài tối đa 80% toàn nến
SPREAD_MAX = 0.15  # Spread tối đa cho phép (0.12-0.15)
VOLUME_INCREASE_MIN = 1.2  # Volume tăng tối thiểu 20% (1.2x)
VOLUME_INCREASE_MAX = 1.4  # Volume tăng tối đa 40% (1.4x)
...
# ==============================================================================
# 5.5. HÀM KIỂM TRA "VÀO LỆNH CHUẨN NHẤT" - 3 ĐIỀU KIỆN
# ==============================================================================

def check_price_structure(df_m1, h1_trend):
    """
    Kiểm tra cấu trúc giá (Structure) - Điều kiện 1
    
    BUY: Giá tạo Higher Low (HL) hoặc vừa phá đỉnh nhỏ (micro breakout)
    SELL: Giá tạo Lower High (LH) hoặc vừa phá đáy nhỏ
    ...
    # Lấy các điểm cao/thấp trong lookback period
    lookback = STRUCTURE_LOOKBACK
    recent_highs = df_m1['high'].iloc[-lookback:].values
    recent_lows = df_m1['low'].iloc[-lookback:].values
    
    # Tìm đỉnh/đáy gần nhất
    current_high = df_m1['high'].iloc[-1]
    current_low = df_m1['low'].iloc[-1]
    prev_high = df_m1['high'].iloc[-lookback:-1].max()
    prev_low = df_m1['low'].iloc[-lookback:-1].min()
    
    # Tìm micro high/low (đỉnh/đáy trong 5 nến gần nhất)
    micro_lookback = 5
    ...
    if h1_trend == 'BUY':
        # BUY: Kiểm tra Higher Low hoặc phá đỉnh nhỏ
        # Higher Low: Đáy hiện tại > đáy trước đó
        has_higher_low = current_low > prev_low
        
        # Micro breakout: Giá phá đỉnh nhỏ
        ...
        if has_higher_low or has_micro_breakout:
            ...
            print(f"    ⚠️ [STRUCTURE] BUY: Chưa có Higher Low hoặc Micro Breakout")
            return 'NONE'
    
    elif h1_trend == 'SELL':
        # SELL: Kiểm tra Lower High hoặc phá đáy nhỏ
        # Lower High: Đỉnh hiện tại < đỉnh trước đó
        has_lower_high = current_high < prev_high
        
        # Micro breakdown: Giá phá đáy nhỏ
        current_close = df_m1['close'].iloc[-1]
        has_micro_breakdown = current_close < micro_low
        
        if has_lower_high or has_micro_breakdown:
            print(f"    ✅ [STRUCTURE] SELL: Lower High={has_lower_high}, Micro Breakdown={has_micro_breakdown}")
            return 'SELL'
        else:
            print(f"    ⚠️ [STRUCTURE] SELL: Chưa có Lower High hoặc Micro Breakdown")
            return 'NONE'
    
    return 'NONE'

def check_momentum(df_m1):
    """
    Kiểm tra Momentum (Năng lượng giá) - Điều kiện 2
    
    Momentum mạnh khi:
    - Nến M1 xanh/đỏ dài > 50-80% toàn nến
    - EMA9 đang tách ra khỏi EMA21
    - Spread thấp (không quá 0.12-0.15)
    - Volume tăng hơn 20-40% so với nến trước
    
    Args:
        df_m1: DataFrame M1
        
    Returns:
        Tuple (bool, str): (has_momentum, direction)
            - has_momentum: True nếu có momentum mạnh
            - direction: 'BUY', 'SELL', hoặc 'NONE'
    """
    if len(df_m1) < EMA_SLOW + 2:
        return False, 'NONE'
    
    # Lấy nến cuối cùng (vừa đóng)
    last_candle = df_m1.iloc[-1]
    prev_candle = df_m1.iloc[-2] if len(df_m1) > 1 else last_candle
    
    # Tính body và total size
    ...
    # Kiểm tra nến dài (50-80%)
    has_strong_candle = CANDLE_BODY_MIN_RATIO <= body_ratio <= CANDLE_BODY_MAX_RATIO
    ...
    # Kiểm tra EMA tách ra (EMA9 tách khỏi EMA21)
    ema_separation = abs(ema_fast_current - ema_slow_current)
    ema_separation_prev = abs(ema_fast_prev - ema_slow_prev)
    ema_expanding = ema_separation > ema_separation_prev  # EMA đang mở rộng
    ...
    # Kiểm tra volume (tick_volume)
    # MT5 không có volume thực, dùng tick_volume làm proxy
    ...
    if prev_volume > 0:
        ...
        volume_increase = VOLUME_INCREASE_MIN <= volume_ratio <= VOLUME_INCREASE_MAX
    else:
        volume_ratio = 1.0  # Default value for display
        volume_increase = True  # Không có dữ liệu volume trước đó → cho phép
    
    # Xác định hướng momentum
    is_bullish = last_candle['close'] > last_candle['open']  # Nến xanh
    is_bearish = last_candle['close'] < last_candle['open']  # Nến đỏ
    
    # EMA tách ra theo hướng
    ema_bullish = ema_fast_current > ema_slow_current and ema_expanding
    ema_bearish = ema_fast_current < ema_slow_current and ema_expanding
    
    # Tổng hợp điều kiện momentum
    if is_bullish and has_strong_candle and ema_bullish and spread_ok and volume_increase:
        print(f"    ✅ [MOMENTUM] BUY: Body={body_ratio:.1%}, EMA tách={ema_expanding}, Spread={spread:.5f}, Volume={volume_ratio:.2f}x")
        return True, 'BUY'
    elif is_bearish and has_strong_candle and ema_bearish and spread_ok and volume_increase:
        print(f"    ✅ [MOMENTUM] SELL: Body={body_ratio:.1%}, EMA tách={ema_expanding}, Spread={spread:.5f}, Volume={volume_ratio:.2f}x")
        return True, 'SELL'
    else:
        reasons = []
        if not has_strong_candle:
            reasons.append(f"Body={body_ratio:.1%} (cần {CANDLE_BODY_MIN_RATIO:.0%}-{CANDLE_BODY_MAX_RATIO:.0%})")
        if not ema_expanding:
            reasons.append("EMA không tách")
        if not spread_ok:
            reasons.append(f"Spread={spread:.5f} > {SPREAD_MAX}")
        if not volume_increase:
            reasons.append(f"Volume={volume_ratio:.2f}x (cần {VOLUME_INCREASE_MIN}-{VOLUME_INCREASE_MAX}x)")
        print(f"    ⚠️ [MOMENTUM] Yếu: {', '.join(reasons)}")
        return False, 'NONE'

def check_confirmation(df_m1, h1_trend):
    """
    Kiểm tra Xác nhận (Confirmation) - Điều kiện 3
    
    BUY trigger:
    - Nến xanh break khỏi vùng kháng cự nhỏ (micro high)
    - Giá đóng lại trên EMA9 sau khi chạm EMA21 (retest thành công)
    - Nến retest tạo đuôi dưới dài hơn thân → rejection BUY
    
    SELL trigger:
    - Nến đỏ break đáy nhỏ
    - Giá đóng dưới EMA9 sau retest EMA21
    - Nến rejection có đuôi trên dài
    ...
    # Tìm micro high/low (5 nến gần nhất)
    micro_lookback = 5
    micro_high = df_m1['high'].iloc[-micro_lookback:-1].max() if len(df_m1) > micro_lookback else current_high
    micro_low = df_m1['low'].iloc[-micro_lookback:-1].min() if len(df_m1) > micro_lookback else current_low
    ...
    if h1_trend == 'BUY':
        # BUY triggers
        # 1. Nến xanh break micro high
        is_green = current_close > current_open
        breaks_micro_high = current_close > micro_high
        
        # 2. Giá đóng trên EMA9 sau retest EMA21
        closes_above_ema9 = current_close > ema_fast_current
        # Kiểm tra xem giá có chạm EMA21 không (retest)
        touched_ema21 = current_low <= ema_slow_current <= current_high
        
        # 3. Nến rejection có đuôi dưới dài
        has_long_lower_wick = lower_wick > body_size and total_size > 0
        
        if (is_green and breaks_micro_high) or (closes_above_ema9 and touched_ema21) or has_long_lower_wick:
            trigger_type = []
            if is_green and breaks_micro_high:
                trigger_type.append("Break micro high")
            if closes_above_ema9 and touched_ema21:
                trigger_type.append("Close above EMA9 after retest EMA21")
            if has_long_lower_wick:
                trigger_type.append("Long lower wick rejection")
            print(f"    ✅ [CONFIRMATION] BUY: {', '.join(trigger_type)}")
            return 'BUY'
        else:
            print(f"    ⚠️ [CONFIRMATION] BUY: Chưa có trigger (Break={breaks_micro_high}, Close above EMA9={closes_above_ema9}, Rejection={has_long_lower_wick})")
            return 'NONE'
    
    elif h1_trend == 'SELL':
        # SELL triggers
        # 1. Nến đỏ break micro low
        is_red = current_close < current_open
        breaks_micro_low = current_close < micro_low
        
        # 2. Giá đóng dưới EMA9 sau retest EMA21
        closes_below_ema9 = current_close < ema_fast_current
        # Kiểm tra xem giá có chạm EMA21 không (retest)
        touched_ema21 = current_low <= ema_slow_current <= current_high
        
        # 3. Nến rejection có đuôi trên dài
        has_long_upper_wick = upper_wick > body_size and total_size > 0
        
        if (is_red and breaks_micro_low) or (closes_below_ema9 and touched_ema21) or has_long_upper_wick:
            trigger_type = []
            if is_red and breaks_micro_low:
                trigger_type.append("Break micro low")
            if closes_below_ema9 and touched_ema21:
                trigger_type.append("Close below EMA9 after retest EMA21")
            if has_long_upper_wick:
                trigger_type.append("Long upper wick rejection")
            print(f"    ✅ [CONFIRMATION] SELL: {', '.join(trigger_type)}")
            return 'SELL'
        else:
            print(f"    ⚠️ [CONFIRMATION] SELL: Chưa có trigger (Break={breaks_micro_low}, Close below EMA9={closes_below_ema9}, Rejection={has_long_upper_wick})")
            return 'NONE'
    
    return 'NONE'

def check_golden_entry(df_m1, h1_trend):
    """
    Kiểm tra "VÀO LỆNH CHUẨN NHẤT" - Phải có đủ 3 điều kiện cùng lúc:
    1. Cấu trúc giá (Structure)
    2. Momentum (Năng lượng giá)
    ...
    # Điều kiện 1: Cấu trúc giá
    structure_signal = check_price_structure(df_m1, h1_trend)
    
    # Điều kiện 2: Momentum
    has_momentum, momentum_direction = check_momentum(df_m1)
...
def run_bot():
    ...
    if ENABLE_GOLDEN_ENTRY:
        print("   2. [GOLDEN ENTRY] Vào lệnh khi đủ 3 điều kiện:")
        print("      - Cấu trúc giá (Higher Low/Lower High hoặc Micro Breakout)")
        print("      - Momentum mạnh (Nến dài 50-80%, EMA9 tách khỏi EMA21, Spread thấp, Volume tăng)")
        print("      - Xác nhận (Break micro high/low, Close trên/dưới EMA9 sau retest EMA21, hoặc Rejection)")
    else:
        print("   2. Chọn điểm vào ở M1 khi giá RETEST lại EMA20 hoặc BREAKOUT")
    print("   3. TP 10–20 pip, SL 8–15 pip (hoặc theo ATR)")
    ...
    while True:
        ...
        # 3. Kiểm tra điểm vào ở M1: RETEST, BREAKOUT, hoặc GOLDEN ENTRY
        print(f"\n  ┌─ [BƯỚC 3] Kiểm tra tín hiệu M1")
        ...
        # Nếu không có Golden Entry, dùng logic cũ (RETEST hoặc BREAKOUT)
        if golden_entry_signal == 'NONE':
            ...
            # Có Golden Entry signal → dùng nó
            m1_signal = golden_entry_signal
            m1_retest_signal = 'NONE'
            m1_breakout_signal = 'NONE'
        ...
        if open_positions <1:
            ...
            # - GOLDEN_ENTRY: Không cần check ADX (đã có momentum check)
            ...
            if signal_type == "GOLDEN_ENTRY":
                # Golden Entry đã có đủ 3 điều kiện, không cần check ADX thêm
                pass
                ...
                m1_signal = 'NONE'  # Block signal
            
            if m1_signal == 'BUY' and h1_trend == 'BUY':
                print(f"  ✅ [QUYẾT ĐỊNH] 🚀 TÍN HIỆU MUA MẠNH!")
                print(f"     - H1 Trend: {h1_trend} (Giá > EMA50)")
                print(f"     - M1 Signal: {m1_signal} ({signal_type})")
                if signal_type == "GOLDEN_ENTRY":
                    print(f"       → Golden Entry: Đủ 3 điều kiện (Structure + Momentum + Confirmation)")
