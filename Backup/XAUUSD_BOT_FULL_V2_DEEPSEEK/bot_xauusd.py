...
class XAUUSD_Bot:
            ...
            if result:
                if result.retcode == mt5.TRADE_RETCODE_DONE:
                    ...
                    if hasattr(self, 'last_tp_price'):
                        self.last_tp_price[result.order] = tp_price
    ...
    def _close_position(self, ticket, reason):
        ...
        if result and result.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            if hasattr(self, 'breakeven_activated') and ticket in self.breakeven_activated:
                self.breakeven_activated.discard(ticket)
            if hasattr(self, 'partial_close_done') and ticket in self.partial_close_done:
                del self.partial_close_done[ticket]
            if hasattr(self, 'last_trailing_check') and ticket in self.last_trailing_check:
                del self.last_trailing_check[ticket]
            if hasattr(self, 'atr_trailing_first_activation') and ticket in self.atr_trailing_first_activation:
                self.atr_trailing_first_activation.discard(ticket)
            if hasattr(self, 'opposite_signal_count') and ticket in self.opposite_signal_count:
