...
class XAUUSD_RiskManager:
    ...
    def check_trading_time(self):
        ...
        # Theo grok.md: Tránh Asian session (thấp volume) và NY Open (volatility cao)
        for start, end in NO_TRADE_SESSIONS:
            # Xử lý session wrap qua nửa đêm (ví dụ: 19:00-04:00)
            if start > end:  # Session wrap qua nửa đêm
                # Kiểm tra: current_time >= start HOẶC current_time <= end
                if current_time >= start or current_time <= end:
                    return False, f"Trong session cấm {start}-{end} (US/Eastern) - Asian session"
            else:
                # Session bình thường (không wrap)
