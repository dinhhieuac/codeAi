"""
Main file để chạy tất cả các bot Scalp Sideway Supper
Chạy nhiều bot cùng lúc với các config khác nhau

Usage:
    python mainScalpSidewaySupper.py
"""
...
def main():
    # Get the directory where mainScalpSidewaySupper.py is located
    base_dir = os.path.dirname(os.path.abspath(__file__))
    
    # List of config files to run
    configs = [
        os.path.join(base_dir, "configs", "scalp_sideway_supper_xau.json"),  # XAUUSD
        os.path.join(base_dir, "configs", "scalp_sideway_supper_eur.json"),   # EURUSD
        os.path.join(base_dir, "configs", "scalp_sideway_supper_btc.json"),   # BTCUSD
    ]
    
    # Bot script path
    bot_script = os.path.join(base_dir, "scalp_sideway_supper.py")
    
    processes = []
    
    print("="*80)
    print("🚀 Starting Scalp Sideway Supper Bots...")
    print("="*80)
