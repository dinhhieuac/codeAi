...
# ==============================================================================
# V3 FUNCTIONS - LIQUIDITY SWEEP, DISPLACEMENT, EXTERNAL BOS
# ==============================================================================

def check_liquidity_sweep_buy(df_m1, atr_val, buffer_pips=2):
    """
    BUY UPDATE 1 - Liquidity Sweep (BẮT BUỘC)
    ...
    # Tìm swing low trước đó (trong 20 nến)
    lookback = min(20, len(df_m1) - 1)
    recent_lows = df_m1.iloc[-lookback:-1]['low'].values
    if len(recent_lows) == 0:
        return False, "Không tìm thấy swing low"
    
    previous_swing_low = min(recent_lows)
    
    # Buffer (2 pips = 0.0002 cho EURUSD)
    buffer = buffer_pips * 0.0001
    
    # Điều kiện 1: current_low < previous_swing_low - buffer
    sweep_condition = current_candle['low'] < (previous_swing_low - buffer)
    
    # Điều kiện 2: lower_wick >= 1.5 × ATR
    lower_wick = min(current_candle['open'], current_candle['close']) - current_candle['low']
    wick_condition = lower_wick >= (1.5 * atr_val)
    
    # Điều kiện 3: close > open (bullish candle)
    bullish_condition = current_candle['close'] > current_candle['open']
    
    if sweep_condition and wick_condition and bullish_condition:
        return True, f"Sweep confirmed: Low {current_candle['low']:.5f} < Swing Low {previous_swing_low:.5f} - {buffer:.5f}"
    else:
        reasons = []
        if not sweep_condition:
            reasons.append(f"Low chưa sweep ({current_candle['low']:.5f} >= {previous_swing_low - buffer:.5f})")
        if not wick_condition:
            reasons.append(f"Lower wick nhỏ ({lower_wick:.5f} < {1.5 * atr_val:.5f})")
        if not bullish_condition:
            reasons.append("Nến không bullish")
        return False, "; ".join(reasons)

def check_liquidity_sweep_sell(df_m1, atr_val, buffer_pips=2):
    """
    SELL UPDATE 1 - Liquidity Sweep
    ...
    # Tìm swing high trước đó (trong 20 nến)
    lookback = min(20, len(df_m1) - 1)
    recent_highs = df_m1.iloc[-lookback:-1]['high'].values
    if len(recent_highs) == 0:
        return False, "Không tìm thấy swing high"
    
    previous_swing_high = max(recent_highs)
    
    # Buffer (2 pips = 0.0002 cho EURUSD)
    buffer = buffer_pips * 0.0001
    
    # Điều kiện 1: current_high > previous_swing_high + buffer
    sweep_condition = current_candle['high'] > (previous_swing_high + buffer)
    
    # Điều kiện 2: upper_wick >= 1.5 × ATR
    upper_wick = current_candle['high'] - max(current_candle['open'], current_candle['close'])
    wick_condition = upper_wick >= (1.5 * atr_val)
    
    # Điều kiện 3: close < open (bearish candle)
    bearish_condition = current_candle['close'] < current_candle['open']
    
    if sweep_condition and wick_condition and bearish_condition:
        return True, f"Sweep confirmed: High {current_candle['high']:.5f} > Swing High {previous_swing_high:.5f} + {buffer:.5f}"
    else:
        reasons = []
        if not sweep_condition:
            reasons.append(f"High chưa sweep ({current_candle['high']:.5f} <= {previous_swing_high + buffer:.5f})")
        if not wick_condition:
            reasons.append(f"Upper wick nhỏ ({upper_wick:.5f} < {1.5 * atr_val:.5f})")
        if not bearish_condition:
            reasons.append("Nến không bearish")
        return False, "; ".join(reasons)

def check_displacement_candle(df_m1, atr_val, signal_type="BUY"):
    """
    BUY/SELL UPDATE 2 - Displacement Candle
    BUY: IF breakout_body >= 1.2 × ATR AND close > previous_range_high
    SELL: IF breakout_body >= 1.2 × ATR AND close < previous_range_low
    """
    if len(df_m1) < 10:
        return False, "Không đủ dữ liệu"
    ...
    # Điều kiện 1: breakout_body >= 1.2 × ATR
    body_condition = body_size >= (1.2 * atr_val)
    
    if not body_condition:
        return False, f"Body nhỏ ({body_size:.5f} < {1.2 * atr_val:.5f})"
    
    # Điều kiện 2: Breakout khỏi range trước đó
    lookback = min(10, len(df_m1) - 1)
    previous_range = df_m1.iloc[-lookback:-1]
    previous_range_high = previous_range['high'].max()
    previous_range_low = previous_range['low'].min()
    
    if signal_type == "BUY":
        breakout_condition = current_candle['close'] > previous_range_high
        if breakout_condition:
            return True, f"Displacement BUY: Close {current_candle['close']:.5f} > Range High {previous_range_high:.5f}"
        else:
            return False, f"Chưa breakout: Close {current_candle['close']:.5f} <= Range High {previous_range_high:.5f}"
    else:  # SELL
        breakout_condition = current_candle['close'] < previous_range_low
        if breakout_condition:
            return True, f"Displacement SELL: Close {current_candle['close']:.5f} < Range Low {previous_range_low:.5f}"
        else:
            return False, f"Chưa breakout: Close {current_candle['close']:.5f} >= Range Low {previous_range_low:.5f}"

def check_external_bos(df_m1, signal_type="BUY", lookback=50):
    """
    BUY/SELL UPDATE 3 - External BOS Filter
    BUY: IF close > last_external_swing_high → BOS_UP_CONFIRMED
    SELL: IF close < last_external_swing_low → BOS_DOWN_CONFIRMED
    ...
    # Tìm external swing (swing lớn hơn, không phải internal)
    # Tìm swing high/low trong lookback period
    df_lookback = df_m1.iloc[-lookback:-1]
    
    if signal_type == "BUY":
        # Tìm swing highs
        swing_highs = []
        for i in range(5, len(df_lookback) - 5):
            is_swing_high = True
            current_high = df_lookback.iloc[i]['high']
            for j in range(i-5, i+6):
                if j != i and j >= 0 and j < len(df_lookback):
                    if df_lookback.iloc[j]['high'] >= current_high:
                        is_swing_high = False
                        break
            if is_swing_high:
        ...
        if len(swing_highs) == 0:
            return False, "Không tìm thấy external swing high"
        
        last_external_swing_high = max(swing_highs)
        
        if current_candle['close'] > last_external_swing_high:
            return True, f"External BOS UP: Close {current_candle['close']:.5f} > External Swing High {last_external_swing_high:.5f}"
        else:
            return False, f"Internal BOS: Close {current_candle['close']:.5f} <= External Swing High {last_external_swing_high:.5f}"
        ...
        for i in range(5, len(df_lookback) - 5):
            is_swing_low = True
            current_low = df_lookback.iloc[i]['low']
            for j in range(i-5, i+6):
                if j != i and j >= 0 and j < len(df_lookback):
                    if df_lookback.iloc[j]['low'] <= current_low:
                        is_swing_low = False
                        break
            if is_swing_low:
        ...
        if len(swing_lows) == 0:
            return False, "Không tìm thấy external swing low"
        
        last_external_swing_low = min(swing_lows)
        
        if current_candle['close'] < last_external_swing_low:
            return True, f"External BOS DOWN: Close {current_candle['close']:.5f} < External Swing Low {last_external_swing_low:.5f}"
        else:
            return False, f"Internal BOS: Close {current_candle['close']:.5f} >= External Swing Low {last_external_swing_low:.5f}"

def check_liquidity_filter(df_m1, entry_price, signal_type="BUY", min_distance_pips=2.5):
    """
    BUY/SELL UPDATE 4 - Liquidity Filter
    BUY: IF distance(entry, nearest_low) < 2.5 pips → WAIT
    SELL: IF distance(entry, nearest_high) < 2.5 pips → WAIT
    """
    if len(df_m1) < 10:
        return True, "Không đủ dữ liệu"
    
    lookback = min(20, len(df_m1) - 1)
    recent_data = df_m1.iloc[-lookback:-1]
    ...
    if signal_type == "BUY":
        ...
        if distance < min_distance:
            return False, f"Liquidity quá gần: Entry {entry_price:.5f} - Nearest Low {nearest_low:.5f} = {distance:.5f} < {min_distance:.5f}"
        else:
            return True, f"Liquidity OK: Distance {distance:.5f} >= {min_distance:.5f}"
        ...
        if distance < min_distance:
            return False, f"Liquidity quá gần: Nearest High {nearest_high:.5f} - Entry {entry_price:.5f} = {distance:.5f} < {min_distance:.5f}"
        else:
            return True, f"Liquidity OK: Distance {distance:.5f} >= {min_distance:.5f}"
...
def check_chop_range(df_m1, atr_val, lookback=10, body_threshold=0.5, overlap_threshold=0.7):
    ...
    if body_condition and overlap_condition:
        return True, f"CHOP detected: Body avg {body_avg:.5f} < {body_threshold * atr_val:.5f}, Overlap {avg_overlap:.2%} > {overlap_threshold:.2%}"
    else:
        reasons = []
        if not body_condition:
            reasons.append(f"Body avg OK ({body_avg:.5f} >= {body_threshold * atr_val:.5f})")
        if not overlap_condition:
            reasons.append(f"Overlap OK ({avg_overlap:.2%} <= {overlap_threshold:.2%})")
        return False, "; ".join(reasons) if reasons else "Không phải CHOP"
...
def tuyen_trend_logic(config, error_count=0):
    ...
    if not skip_strategy_eval:
        ...
        if has_enough_signals and is_touch and pass_fib and is_smooth:
            ...
            # V3: Check V3 conditions before confirming signal
            print(f"\n{'='*80}")
            print(f"🔍 [V3 FILTERS - Tránh sóng ngắn]")
            print(f"{'='*80}")
            
            # Calculate ATR for V3 checks
            atr_val = calculate_atr(df_m1, 14).iloc[-1] if len(df_m1) >= 14 else 0.0001
            
            # 1. Check CHOP/RANGE (chung cho cả BUY và SELL)
            is_chop, chop_msg = check_chop_range(df_m1, atr_val)
            if is_chop:
            ...
            if signal_type == "BUY":
                sweep_ok, sweep_msg = check_liquidity_sweep_buy(df_m1, atr_val)
                if not sweep_ok:
                ...
                sweep_ok, sweep_msg = check_liquidity_sweep_sell(df_m1, atr_val)
                if not sweep_ok:
                    print(f"   ❌ Liquidity Sweep FAIL: {sweep_msg}")
                    signal_type = None
                    log_details.append(f"V3 Filter: No liquidity sweep - {sweep_msg}")
            ...
            # 3. Check Displacement Candle
            if signal_type:
                displacement_ok, displacement_msg = check_displacement_candle(df_m1, atr_val, signal_type)
                if not displacement_ok:
                    print(f"   ❌ Displacement FAIL: {displacement_msg}")
                    signal_type = None
                    log_details.append(f"V3 Filter: No displacement - {displacement_msg}")
                else:
                    print(f"   ✅ Displacement OK: {displacement_msg}")
            
            # 4. Check External BOS
            if signal_type:
                bos_ok, bos_msg = check_external_bos(df_m1, signal_type)
                if not bos_ok:
                    print(f"   ❌ External BOS FAIL: {bos_msg}")
                    signal_type = None
                    log_details.append(f"V3 Filter: Internal BOS only - {bos_msg}")
                else:
                    print(f"   ✅ External BOS OK: {bos_msg}")
        else:
    ...
    # V3: Check Liquidity Filter before execution (UPDATE 4)
    if signal_type:
        print(f"\n{'='*80}")
        print(f"🔍 [V3 LIQUIDITY FILTER - Tránh liquidity quá gần]")
        print(f"{'='*80}")
        liquidity_ok, liquidity_msg = check_liquidity_filter(df_m1, price, signal_type)
        if not liquidity_ok:
            print(f"   ❌ Liquidity Filter FAIL: {liquidity_msg}")
            signal_type = None
            log_details.append(f"V3 Filter: Liquidity too close - {liquidity_msg}")
        else:
            print(f"   ✅ Liquidity Filter OK: {liquidity_msg}")
