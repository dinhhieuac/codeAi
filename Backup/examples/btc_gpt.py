...
# --- AUTO-TRADE HELPERS (added) ---
def _write_trade_log(log_path: str, entry: str, append: bool = True):
    ...
    try:
        ...
        try:
            ...
            _lg.exception("Failed to write trade log: %s", e)
        except:
            pass

def _compute_lot_size(base_lot: float, strength: float):
    try:
        if strength >= 6:
            return round(base_lot * 1.5, 6)
        elif strength <= 3:
            return round(base_lot * 0.7, 6)
        else:
            return round(base_lot, 6)
    except:
        return base_lot
# --- END HELPERS ---
...
class GoldAutoTrader:
    ...
    def analyze_market(self, df: pd.DataFrame) -> Dict[str, any]:
        """Phân tích thị trường và tạo tín hiệu (Aggressive strategy).

        Luật (Aggressive):
        - BUY nếu (RSI < 35 hoặc StochK < 25) và/hoặc giá chạm Bollinger Lower
        - SELL nếu (RSI > 65 hoặc StochK > 75) và/hoặc giá chạm Bollinger Upper
        - Không yêu cầu bắt buộc MACD/Trend; MACD chỉ tăng weight khi histogram bắt đầu tăng/giảm.
        - Trả về: {'signal': 'BUY'/'SELL'/'HOLD', 'strength': int, 'reason': str, 'details': {...}}
        """
        if len(df) < 50:
            return {'signal': 'HOLD', 'strength': 0, 'reason': 'Không đủ dữ liệu', 'details': {}}
        ...
        macd_hist_prev = float(macd['hist'].iloc[-2]) if len(macd['hist']) >= 2 else 0.0
        details['macd_hist'] = macd_hist
        details['macd_hist_prev'] = macd_hist_prev

        # MA trend info (optional, used for weight only)
        ma20 = df['close'].rolling(window=20).mean().iloc[-1] if len(df) >= 20 else np.nan
        ma50 = df['close'].rolling(window=50).mean().iloc[-1] if len(df) >= 50 else np.nan
        details['ma20'] = float(ma20) if not np.isnan(ma20) else None
        details['ma50'] = float(ma50) if not np.isnan(ma50) else None

        signal = 'HOLD'
        strength = 0

        # Aggressive BUY conditions
        rsi_buy = rsi_current < max(35, getattr(self, 'rsi_oversold', 35))
        stoch_buy = stoch_k < 25
        ...
        if rsi_buy:
            strength += 2
            reasons.append(f'RSI low ({rsi_current:.2f})')
        if stoch_buy:
            strength += 2
            reasons.append(f'Stoch oversold K={stoch_k:.2f}')
        if bb_buy:
            strength += 3
            reasons.append('Price at/under Bollinger Lower')
        if macd_hist_increasing and macd_hist > 0:
            strength += 1
            reasons.append('MACD histogram increasing (positive momentum)')
        elif macd_hist_increasing:
            # histogram increasing but still negative -> small weight
            strength += 0.5
            reasons.append('MACD histogram increasing (still negative)')

        # Aggressive SELL conditions (mirror)
        rsi_sell = rsi_current > min(65, getattr(self, 'rsi_overbought', 65))
        stoch_sell = stoch_k > 75
        bb_sell = price >= bb_upper
        macd_hist_decreasing = macd_hist < macd_hist_prev
        ...
        if rsi_sell:
            sell_strength += 2
            sell_reasons.append(f'RSI high ({rsi_current:.2f})')
        if stoch_sell:
            sell_strength += 2
            sell_reasons.append(f'Stoch overbought K={stoch_k:.2f}')
        if bb_sell:
            sell_strength += 3
            sell_reasons.append('Price at/over Bollinger Upper')
        if macd_hist_decreasing and macd_hist < 0:
            sell_strength += 1
            sell_reasons.append('MACD histogram decreasing (negative momentum)')
        elif macd_hist_decreasing:
            sell_strength += 0.5
            sell_reasons.append('MACD histogram decreasing (still positive)')

        # Decide final signal by comparing strengths but prefer the side with absolute threshold
        # Aggressive threshold: any side with strength >= 3 triggers a signal
        details['buy_score'] = strength
        details['sell_score'] = sell_strength

        if strength >= 3 and strength > sell_strength:
            signal = 'BUY'
            reason = '; '.join(reasons)
        elif sell_strength >= 3 and sell_strength > strength:
            signal = 'SELL'
            reason = '; '.join(sell_reasons)
        else:
            signal = 'HOLD'
            reason = 'No clear aggressive signal; scores -> BUY: {:.2f}, SELL: {:.2f}'.format(strength, sell_strength)
            # still include reasons if any exist
            if reasons or sell_reasons:
                reason += ' | ' + '; '.join(reasons + sell_reasons)

        # Strength normalization (cap)
        strength = float(max(strength, sell_strength)) if signal != 'HOLD' else 0.0

        
        # Auto lot sizing based on strength
        base_lot = getattr(self, 'min_lot', 0.01)
        chosen_strength = strength if signal == 'BUY' else (sell_strength if signal == 'SELL' else 0.0)
        lot = _compute_lot_size(base_lot, chosen_strength)
        details['lot_size'] = float(lot)
        details['base_lot'] = float(base_lot)
            ...
            while True:
                ...
                if num_positions < self.max_positions and self._check_daily_trade_limit():
                    ...
                    if df is not None:
                        ...
                        reason_str = analysis.get('reason', 'No signals')
                        details = analysis.get('details', {})
                        
                        logger.info(f"Phan tich ({self.timeframe_str}): Signal={analysis['signal']}, Strength={analysis['strength']}")
                        logger.info(f"   RSI: {details.get('rsi', 'N/A'):.2f}" if details.get('rsi') is not None else "   RSI: N/A")
                        logger.info(f"   Stoch K: {details.get('stoch_k', 'N/A'):.2f}" if details.get('stoch_k') is not None else "   Stoch K: N/A")
                        logger.info(f"   Buy score: {details.get('buy_score', 0):.2f}, Sell score: {details.get('sell_score', 0):.2f}")
                        ...
                        if analysis['signal'] == 'HOLD':
                            ...
                            logger.info(f"   DEBUG: Buy score: {buy_score:.2f}, Sell score: {sell_score:.2f} (cần >= 3)")
