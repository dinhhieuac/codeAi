"""
Gold GPT Trader - Hệ thống giao dịch tự động cho vàng (XAUUSD) theo rules_gold_usd.md
Tự động phân tích và thực thi lệnh Buy/Sell dựa trên phân tích kỹ thuật M15
Tính năng: Trading hours, Lot size động, Trailing stop, Breakeven, SL/TP linh hoạt
"""
...
try:
    ...
    MIN_LOT_SIZE = 0.01
    MAX_LOT_SIZE = 0.1
    BASE_LOT_SIZE = 0.01  # Lot size cơ bản
    MAX_POSITIONS = 2  # Theo rules
    MAX_DAILY_TRADES = 10  # Theo rules
    MIN_EQUITY_RATIO = 0.85  # Equity < 85% Balance → dừng
    USE_ATR_SL_TP = True
    ATR_SL_MULTIPLIER = 1.0  # Theo rules: SL = 1.0 × ATR
    ATR_TP_MULTIPLIER = 1.5  # Theo rules: TP = 1.5 × ATR
    MIN_SL_POINTS = 200
    MAX_SL_POINTS = 1000
    MIN_TP_POINTS = 300
    MAX_TP_POINTS = 2000
    ...
    INTERVAL_SECONDS = 60
    HISTORICAL_BARS = 200
    MAGIC_NUMBER = 888886  # Magic number riêng cho gold_gpt
    BUY_COMMENT = "GoldGPT Buy"
    SELL_COMMENT = "GoldGPT Sell"
    LOG_LEVEL = "INFO"
    LOG_FILE = "logs/gold_gpt_trader.log"
    CSV_LOG_FILE = "logs/trades_gold_gpt.csv"
    DEVIATION = 10
...
class GoldGPTrader:
    """
    Hệ thống giao dịch tự động cho XAUUSD theo rules_gold_usd.md
    
    Tự động phân tích thị trường bằng các chỉ báo kỹ thuật (RSI, MACD, MA, BB, Volume)
    và thực thi lệnh Buy/Sell dựa trên tín hiệu từ nhiều chỉ báo đồng thuận.
    
    Tính năng theo rules:
    - Trading Hours: Chỉ trade trong phiên Âu (14:00-17:30) và Mỹ (19:30-23:30)
    - ATR-based SL/TP: SL = 1.0×ATR, TP = 1.5×ATR (cơ bản)
    - SL/TP linh hoạt: Tại S/R giảm TP xuống 1.0×ATR, SL 0.8×ATR
    - Trend mạnh: TP lên 2×ATR, SL giữ 1×ATR
    - Lot size động: Giảm 50% sau 3 loss, tăng 25% sau 2 win
    - Trailing Stop: Kích hoạt khi profit > 1×ATR, theo sau 50% profit
    - Auto Breakeven: Khi profit đạt 1.2×SL → dời SL về entry
    - Time Rules: 45 phút cùng chiều, 20 phút ngược chiều
    """
    ...
    def __init__(self, login: int, password: str, server: str, symbol: str = "XAUUSD"):
        ...
        # Cấu hình giao dịch - Risk Management (load từ configgold.py theo rules)
        self.risk_per_trade = globals().get('RISK_PER_TRADE', 0.0075)  # 0.75% (0.5-1% theo rules)
        self.min_lot = globals().get('MIN_LOT_SIZE', 0.01)  # Lot size tối thiểu
        self.max_lot = globals().get('MAX_LOT_SIZE', 0.1)  # Lot size tối đa
        self.base_lot = globals().get('BASE_LOT_SIZE', 0.01)  # Lot size cơ bản (theo rules)
        self.max_positions = globals().get('MAX_POSITIONS', 2)  # Theo rules: 2
        self.max_daily_trades = globals().get('MAX_DAILY_TRADES', 10)  # Theo rules: 10
        self.min_equity_ratio = globals().get('MIN_EQUITY_RATIO', 0.85)  # Theo rules: 85%
        ...
        # SL/TP Settings - Cài đặt Stop Loss và Take Profit theo rules_gold_usd.md
        self.use_atr_sl_tp = globals().get('USE_ATR_SL_TP', True)  # True: Dùng ATR
        self.atr_sl_multiplier = globals().get('ATR_SL_MULTIPLIER', 1.0)  # Theo rules: SL = 1.0 × ATR
        self.atr_tp_multiplier = globals().get('ATR_TP_MULTIPLIER', 1.5)  # Theo rules: TP = 1.5 × ATR
        ...
        # Kiểm tra biến có tồn tại trong globals() (đã import từ configbtc)
        ...
        # Trading Time Rules (theo rules_gold_usd.md)
        self.min_time_same_direction = globals().get('MIN_TIME_BETWEEN_SAME_DIRECTION', 45 * 60)  # Theo rules: 45 phút
        self.min_time_opposite_direction = globals().get('MIN_TIME_BETWEEN_OPPOSITE_DIRECTION', 20 * 60)  # Theo rules: 20 phút
        ...
        # Lot size động theo rules
        self.current_lot_multiplier = 1.0  # Hệ số nhân lot size hiện tại
        self.consecutive_losses = 0  # Số lệnh thua liên tiếp
        self.consecutive_wins = 0  # Số lệnh thắng liên tiếp
        
        # Trading hours theo rules (giờ VN)
        self.trading_hours_eu = (dt_time(14, 0), dt_time(17, 30))  # Phiên Âu: 14:00-17:30
        self.trading_hours_us = (dt_time(19, 30), dt_time(23, 30))  # Phiên Mỹ: 19:30-23:30
        self.avoid_hours_1 = (dt_time(14, 30), dt_time(16, 30))  # Tránh: 14:30-16:30
        self.avoid_hours_2 = (dt_time(20, 30), dt_time(21, 30))  # Tránh: 20:30-21:30 (tin tức Mỹ)
        
        # Trailing stop và Breakeven tracking
        self.trailing_stops = {}  # Dict: ticket -> trailing_stop_info
    ...
    def _check_trading_hours(self) -> bool:
        """
        Kiểm tra có trong giờ giao dịch tốt không (theo rules_gold_usd.md)
        
        Returns:
            True nếu trong giờ giao dịch tốt, False nếu không
        """
        now = datetime.now()
        current_time = now.time()
        
        # Kiểm tra trong giờ tránh
        if (self.avoid_hours_1[0] <= current_time <= self.avoid_hours_1[1] or
            self.avoid_hours_2[0] <= current_time <= self.avoid_hours_2[1]):
            logger.debug(f"⏸️ Đang trong giờ tránh: {current_time.strftime('%H:%M')}")
            return False
        
        # Kiểm tra trong giờ giao dịch tốt
        in_eu_session = (self.trading_hours_eu[0] <= current_time <= self.trading_hours_eu[1])
        in_us_session = (self.trading_hours_us[0] <= current_time <= self.trading_hours_us[1])
        
        if in_eu_session or in_us_session:
            return True
        
        logger.debug(f"⏸️ Ngoài giờ giao dịch: {current_time.strftime('%H:%M')} (Âu: 14:00-17:30, Mỹ: 19:30-23:30)")
        return False
    
    def _update_lot_size(self, is_win: bool):
        """
        Cập nhật lot size động theo rules_gold_usd.md
        
        Args:
            is_win: True nếu lệnh thắng, False nếu thua
        """
        if is_win:
            self.consecutive_wins += 1
            self.consecutive_losses = 0
            
            # Tăng 25% sau 2 lệnh thắng liên tiếp, nhưng không vượt 1.5×
            if self.consecutive_wins >= 2:
                new_multiplier = min(1.5, self.current_lot_multiplier * 1.25)
                if new_multiplier > self.current_lot_multiplier:
                    self.current_lot_multiplier = new_multiplier
                    logger.info(f"📈 Lot size tăng: {self.current_lot_multiplier:.2f}x (sau {self.consecutive_wins} lệnh thắng)")
        else:
            self.consecutive_losses += 1
            self.consecutive_wins = 0
            
            # Giảm 50% sau 3 lệnh thua liên tiếp
            if self.consecutive_losses >= 3:
                self.current_lot_multiplier = max(0.5, self.current_lot_multiplier * 0.5)
                logger.warning(f"📉 Lot size giảm: {self.current_lot_multiplier:.2f}x (sau {self.consecutive_losses} lệnh thua)")
                self.consecutive_losses = 0  # Reset sau khi giảm
    
    def _get_current_lot_size(self) -> float:
        """
        Lấy lot size hiện tại với multiplier động
        
        Returns:
            Lot size đã điều chỉnh
        """
        lot_size = self.base_lot * self.current_lot_multiplier
        return max(self.min_lot, min(lot_size, self.max_lot))
    ...
    def _manage_trailing_stops(self):
        """
        Quản lý Trailing Stop theo rules_gold_usd.md
        - Kích hoạt khi profit > 1×ATR
        - Theo sau 50% profit hiện tại
        """
        ...
        for pos in positions:
            ...
            profit_atr = profit_points * point / atr_current
            
            # Kích hoạt khi profit > 1×ATR
            if profit_atr > 1.0:
                # Tính SL mới: theo sau 50% profit
                new_sl_distance = profit_points * 0.5  # 50% profit
                
                if pos.type == mt5.ORDER_TYPE_BUY:
                    new_sl = entry_price + (new_sl_distance * point)
                    ...
                    if new_sl > current_sl:
                        ...
                        if result.retcode == mt5.TRADE_RETCODE_DONE:
                            logger.info(f"📈 Trailing Stop: Ticket {ticket}, SL: {current_sl:.2f} → {new_sl:.2f}")
                else:  # SELL
                    new_sl = entry_price - (new_sl_distance * point)
    ...
    def _manage_breakeven(self):
        """
        Quản lý Auto Breakeven theo rules_gold_usd.md
        - Khi profit đạt 1.2×SL → dời SL về entry (breakeven)
        """
        positions = self.get_open_positions()
        if not positions:
            return
        ...
        for pos in positions:
            ...
            entry_price = pos.price_open
            current_sl = pos.sl
            sl_distance = abs(entry_price - current_sl) if current_sl > 0 else 0
            ...
            # Kích hoạt khi profit >= 1.2×SL
            profit_sl_ratio = (profit_points * point) / sl_distance
            if profit_sl_ratio >= 1.2:
                # Dời SL về entry (breakeven)
                request = {
                    "action": mt5.TRADE_ACTION_SLTP,
                    "symbol": self.symbol,
                    "position": ticket,
                ...
                if result.retcode == mt5.TRADE_RETCODE_DONE:
                    ...
                    logger.info(f"✅ Breakeven: Ticket {ticket}, SL: {current_sl:.2f} → {entry_price:.2f} (entry)")
    
    def calculate_risk_parameters(self, df: pd.DataFrame, analysis: Dict = None) -> Tuple[float, float, float]:
        """
        ...
        if self.use_atr_sl_tp:
            ...
            if np.isnan(atr_current) or atr_current == 0:
                ...
                # R2: Sử dụng ATR để tính SL/TP rõ ràng theo rules_gold_usd.md
                ...
                # ⚠️ LOGIC LINH HOẠT theo rules:
                # 1. Cơ bản: SL = 1.0×ATR, TP = 1.5×ATR
                # 2. Tại S/R: TP = 1.0×ATR, SL = 0.8×ATR
                # 3. Trend mạnh: TP = 2×ATR, SL = 1×ATR
                
                sl_multiplier = self.atr_sl_multiplier  # Mặc định 1.0
                tp_multiplier = self.atr_tp_multiplier  # Mặc định 1.5
                
                # Kiểm tra có ở S/R không (từ analysis)
                if analysis and analysis.get('support_resistance') and analysis['support_resistance'].get('signal'):
                    # Tại S/R: giảm TP xuống 1.0×ATR, SL 0.8×ATR
                    sl_multiplier = 0.8
                    tp_multiplier = 1.0
                    logger.info("📊 Điều chỉnh SL/TP: Tại S/R zone → SL=0.8×ATR, TP=1.0×ATR")
                
                # Kiểm tra trend mạnh (MA20 > MA50 > MA200)
                elif analysis:
                    trend = analysis.get('trend', 'NEUTRAL')
                    if trend == 'UP':
                        # Kiểm tra MA slope và ADX
                        ma_slope = analysis.get('ma_slope', {})
                        adx_info = analysis.get('adx', {})
                        
                        if (ma_slope.get('strength', False) and 
                            adx_info.get('value', 0) and adx_info.get('value', 0) >= 40):
                            # Trend mạnh: TP lên 2×ATR, SL giữ 1×ATR
                            sl_multiplier = 1.0
                            tp_multiplier = 2.0
                            logger.info("📊 Điều chỉnh SL/TP: Trend mạnh → SL=1.0×ATR, TP=2.0×ATR")
                ...
                if use_rr_ratio:
                    ...
                    # Tính TP từ ATR với multiplier đã điều chỉnh
        ...
        # R1: Tính lot size dựa trên risk và lot size động (theo rules_gold_usd.md)
        account_info = mt5.account_info()
        ...
        if tick_value > 0 and sl_points > 0:
            lot_size_from_risk = risk_amount / (sl_points * tick_value)
        else:
            lot_size_from_risk = self.min_lot
        
        # Áp dụng lot size động (multiplier từ consecutive wins/losses)
        lot_size = lot_size_from_risk * self.current_lot_multiplier
    ...
    def place_buy_order(self, lot: float = None, sl_points: float = None, tp_points: float = None, reason: str = "") -> Optional[dict]:
        ...
        if sl_points is None or tp_points is None or lot is None:
            ...
            if df is not None:
                # Tính analysis để truyền vào calculate_risk_parameters (cho SL/TP linh hoạt)
    ...
    def run_auto_trading(self, interval_seconds: int = 60):
        ...
        try:
            while True:
                ...
                # ⚠️ MỚI: Kiểm tra trading hours (theo rules_gold_usd.md)
                if not self._check_trading_hours():
                    logger.debug("⏸️ Ngoài giờ giao dịch tốt, chờ đến phiên tiếp theo...")
