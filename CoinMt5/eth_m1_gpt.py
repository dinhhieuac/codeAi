...
def load_config(filename=None):
    ...
    try:
        ...
        SYMBOL = config.get("SYMBOL", "ETHUSDm") 
...
def get_pip_value():
    """
    Tính giá trị pip cho ETHUSD
    
    ⚠️ VỚI ETHUSD: 
    - 1 lot = 0.1 ETH (khác với forex: 1 lot = 100,000)
    - 1 pip = 1 USD movement trong giá
    - Với lot 0.01 (tức 0.001 ETH): pip value phụ thuộc vào contract size
...
def calculate_atr_from_m1(df_m1, period=14):
    ...
    ⚠️ VỚI ETHUSD: 1 pip = 1 USD = 1 point
...
def send_order(trade_type, volume, df_m1=None, deviation=20):
    ...
    if ENABLE_FIXED_SL_USD and FIXED_SL_USD > 0:
        ...
        # ⚠️ VỚI ETHUSD: 1 pip = 1 USD, nên SL (pips) = SL (USD)
        sl_pips_fixed = FIXED_SL_USD  # Với ETHUSD: 1 USD = 1 pip
        sl_points = sl_pips_fixed  # Với ETHUSD: 1 pip = 1 point
        ...
        if df_m1 is not None:
            ...
            if atr_pips is not None:
                ...
                tp_points = tp_pips  # Với ETHUSD: 1 pip = 1 point
                tp_points = max(TP_POINTS_MIN, min(tp_points, TP_POINTS_MAX))
                tp_pips_limited = tp_points  # Với ETHUSD: points = pips
        ...
        # ⚠️ VỚI ETHUSD: 1 pip = 1 point, ATR đã là pips
        if df_m1 is not None:
            atr_pips = calculate_atr_from_m1(df_m1)
            if atr_pips is not None:
                ...
                # ⚠️ VỚI ETHUSD: 1 pip = 1 point (không cần nhân 10)
                sl_points = sl_pips
                tp_points = tp_pips
...
def manage_positions():
    ...
    for pos in positions:
        ...
        if ENABLE_BREAK_EVEN and BREAK_EVEN_START_POINTS > 0 and profit_points >= BREAK_EVEN_START_POINTS:
            # +1 pip (1 point cho ETHUSD) để bù spread và tránh bị dính SL ngay lập tức
            pips_buffer = 1 * point  # Với ETHUSD: 1 pip = 1 point 
        ...
        if ENABLE_TRAILING_STOP:
            ...
            if profit_points >= tp_half_points and atr_pips is not None:
                ...
                # ⚠️ VỚI ETHUSD: 1 pip = 1 point
                trailing_step_pips = atr_pips * TRAILING_STEP_ATR_MULTIPLIER
                trailing_step_points = trailing_step_pips  # Với ETHUSD: 1 pip = 1 point
