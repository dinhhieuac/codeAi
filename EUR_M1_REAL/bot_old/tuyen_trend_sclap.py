...
def m1_scalp_logic(config, error_count=0):
    """
    M1 Scalp Strategy Logic for EURUSD
    ...
    try:
        ...
        # For EURUSD, ATR threshold should be adjusted
        # Default: 0.0001 for EURUSD (adjust based on typical EURUSD volatility)
        atr_val = curr_candle['atr']
        min_atr = config.get('min_atr', 0.0001)  # Default 0.0001 for EURUSD
        ...
        if signal_type is None:
            ...
            print(f"📊 [M1 Scalp EUR] Price: {curr_candle['close']:.5f} | EMA50: {ema50_val:.5f} | EMA200: {ema200_val:.5f} | RSI: {current_rsi:.1f} | ATR: {atr_val:.5f}")
            return error_count, 0
        ...
        print(f"\n{'='*80}")
        print(f"🚀 [M1 SCALP EUR SIGNAL] {signal_type} @ {current_price:.5f}")
        print(f"{'='*80}")
        ...
        if check_result.retcode != mt5.TRADE_RETCODE_DONE:
            ...
            print(f"❌ {error_msg}")
            send_telegram(
                f"❌ <b>M1 Scalp EUR Bot - Lỗi Gửi Lệnh</b>\n"
        ...
        if result.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            db.log_order(result.order, "M1_Scalp_EUR", symbol, signal_type, volume, current_price, sl, tp, reason, account_id=config.get('account'))
            
            # Detailed Telegram Message
            msg_parts = []
            msg_parts.append(f"✅ <b>M1 Scalp EUR Bot - Lệnh Đã Được Thực Hiện</b>\n")
...
if __name__ == "__main__":
    ...
    if config and connect_mt5(config):
        print("\n" + "="*80)
        print(f"✅ M1 Scalp EUR Bot - Started")
        print(f"💱 Symbol: {config.get('symbol', 'EURUSD')}")
