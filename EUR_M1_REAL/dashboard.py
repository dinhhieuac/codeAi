    ...
    try:
        ...
        if positions:
            for pos in positions:
                ...
                # Check if price is close (within 0.5 points for EURUSD)
                if abs(pos.price_open - signal_price) > 0.0005:
                    continue
        ...
        for deal in deals:
            ...
            # Check if price is close (within 0.5 points for EURUSD)
            if abs(deal.price - signal_price) > 0.0005:
                continue
...
def analyze_signal(signal_id):
    ...
    if result == 'win':
        ...
        if order_info.get('close_price') and signal_dict['sl']:
            ...
            if sl_distance < 0.0001:  # Hit SL (for EURUSD)
                analysis['analysis'].append({
                    'type': 'error',
                    'title': '🛑 Hit Stop Loss',
                ...
                if signal_dict['sl'] and signal_dict['price']:
                    sl_distance_pips = abs(signal_dict['price'] - signal_dict['sl']) * 10000  # For EURUSD
                    if sl_distance_pips < 10:
                        analysis['recommendations'].append({
                            'type': 'warning',
                            'title': '⚠️ SL Too Tight',
                            'message': f"SL distance: ~{sl_distance_pips:.1f} pips. Consider widening SL to at least 10-20 pips to avoid noise."
                        })
                    elif sl_distance_pips > 50:
                        analysis['recommendations'].append({
                            'type': 'info',
                            'title': '💡 SL Too Wide',
            ...
            elif tp_distance < 0.0001:  # Hit TP (shouldn't happen for loss)
                pass
            else:
                analysis['analysis'].append({
                    'type': 'warning',
...
if __name__ == '__main__':
    print(f"🚀 Dashboard running on http://127.0.0.1:5008")
