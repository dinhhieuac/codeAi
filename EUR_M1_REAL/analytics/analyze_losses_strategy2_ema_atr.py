...
def analyze_order_loss(ticket, order_type, open_time_str, open_price, sl, tp, close_price, profit):
    ...
    if len(df_m1_entry) < 28 or len(df_h1_entry) < 50:
    ...
    # M1 EMA 14 and 28
    df_m1_entry['ema14'] = df_m1_entry['close'].ewm(span=14, adjust=False).mean()
    df_m1_entry['ema28'] = df_m1_entry['close'].ewm(span=28, adjust=False).mean()
    ...
    if not has_crossover:
        if order_type == "BUY":
            issues.append(f"❌ EMA Crossover: Không có crossover (EMA14: {prev['ema14']:.3f} → {last['ema14']:.3f}, EMA28: {prev['ema28']:.3f} → {last['ema28']:.3f})")
        else:
            issues.append(f"❌ EMA Crossover: Không có crossover (EMA14: {prev['ema14']:.3f} → {last['ema14']:.3f}, EMA28: {prev['ema28']:.3f} → {last['ema28']:.3f})")
    else:
        if order_type == "BUY":
            passed_conditions.append(f"✅ EMA Crossover: EMA14 > EMA28 (Bullish)")
        else:
            passed_conditions.append(f"✅ EMA Crossover: EMA14 < EMA28 (Bearish)")
    ...
    # Check Price Extension (only if crossover exists)
    if has_crossover:
        ...
        if is_extended:
            issues.append(f"❌ Price Extension: {price_dist:.2f} > {atr_threshold:.2f} (1.5xATR)")
        else:
            passed_conditions.append(f"✅ Price Extension: {price_dist:.2f} <= {atr_threshold:.2f} (1.5xATR)")
    ...
    if len(issues) > 0:
        ...
        print(f"   🤔 Có thể do: SL quá chặt, market reversal, hoặc false crossover")
...
def analyze_order_loss_with_stats(ticket, order_type, open_time_str, open_price, sl, tp, close_price, profit):
    ...
    if order_type == "BUY":
        if not has_crossover:
            stats['issues'].append("EMA_Crossover")
        else:
            stats['passed_conditions'].append("EMA_Crossover")
        ...
        is_extended = price_dist > atr_threshold
        if is_extended:
            stats['issues'].append("Price_Extension")
        else:
            stats['passed_conditions'].append("Price_Extension")
...
def generate_summary_report(all_stats, output_file):
    ...
    if issue_counts:
        for issue, count in sorted(issue_counts.items(), key=lambda x: x[1], reverse=True):
            ...
            issue_name = {
                'EMA_Crossover': 'EMA Crossover không đúng',
                'H1_Trend': 'H1 Trend không đúng',
                'Price_Extension': 'Price Extension (giá quá xa EMA14)',
    ...
    if issue_counts.get('EMA_Crossover', 0) > total_losses * 0.3:
        improvements.append({
            'priority': 'HIGH',
            'issue': 'EMA Crossover không đúng',
            'count': issue_counts['EMA_Crossover'],
            'percentage': issue_percentages['EMA_Crossover'],
            'suggestion': [
                'Kiểm tra lại logic crossover (có thể vào lệnh quá sớm)',
    ...
    if issue_counts.get('Price_Extension', 0) > total_losses * 0.3:
        improvements.append({
            'priority': 'MEDIUM',
            'issue': 'Price Extension (giá quá xa EMA14)',
            'count': issue_counts['Price_Extension'],
            'percentage': issue_percentages['Price_Extension'],
            'suggestion': [
                'Tăng threshold từ 1.5x ATR lên 2.0x ATR',
                'Thêm filter: chỉ trade khi giá gần EMA14 (mean reversion)'
            ]
        })
    ...
    report.append("  1. Kiểm tra lại logic EMA crossover")
    report.append("  2. Tăng H1 trend filter strength")
