...
skip_session = True
def check_asian_session(symbol, allow_eur=False):
    """
    Check if current time is Asian session
    Asian Session: 00:00 - 08:00 GMT
    Ngoại lệ: EUR có thể trade (tuỳ cấu hình)
    """
    if skip_session:
    ...
    if is_asian:
        if allow_eur and ('EUR' in symbol.upper()):
            return False, "Asian session but EUR allowed"
...
def strategy_1_logic_v21(config, error_count=0):
    ...
    # ========================================================================
    allow_eur_asian = config['parameters'].get('allow_eur_asian', False)
    is_asian, session_msg = check_asian_session(symbol, allow_eur=allow_eur_asian)
    if is_asian:
