...
def m1_scalp_logic(config, error_count=0):
    """
    M1 Scalp Strategy Logic for ETHUSD
    ...
    try:
        ...
        # For ETHUSD, ATR threshold should be adjusted (ETH typically has smaller ATR than BTC)
        # Default: 0.00011 for EURUSD, for ETHUSD use 0.5 (adjust based on typical ETH price ~2000-4000)
        atr_val = curr_candle['atr']
        min_atr = config.get('min_atr', 0.5)  # Default 0.5 for ETHUSD
        ...
        if signal_type is None:
            ...
            print(f"📊 [M1 Scalp ETH] Price: {curr_candle['close']:.2f} | EMA50: {ema50_val:.2f} | EMA200: {ema200_val:.2f} | RSI: {current_rsi:.1f} | ATR: {atr_val:.2f}")
            return error_count, 0
        ...
        print(f"\n{'='*80}")
        print(f"🚀 [M1 SCALP ETH SIGNAL] {signal_type} @ {current_price:.2f}")
        print(f"{'='*80}")
        ...
        if check_result.retcode != mt5.TRADE_RETCODE_DONE:
            ...
            print(f"❌ {error_msg}")
            send_telegram(
                f"❌ <b>M1 Scalp ETH Bot - Lỗi Gửi Lệnh</b>\n"
        ...
        if result.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            db.log_order(result.order, "M1_Scalp_ETH", symbol, signal_type, volume, current_price, sl, tp, reason, account_id=config.get('account'))
            
            # Detailed Telegram Message
            msg_parts = []
            msg_parts.append(f"✅ <b>M1 Scalp ETH Bot - Lệnh Đã Được Thực Hiện</b>\n")
...
if __name__ == "__main__":
    ...
    if config and connect_mt5(config):
        print("\n" + "="*80)
        print(f"✅ M1 Scalp ETH Bot - Started")
        print(f"💱 Symbol: {config.get('symbol', 'ETHUSD')}")
