...
def strategy_1_logic(config, error_count=0):
    ...
    if signal:
        ...
        if sl_mode == 'auto_m5':
            ...
            # Add a small buffer (e.g., 200 points / $2) to avoid noise
            buffer = 200 * mt5.symbol_info(symbol).point
            
            if signal == "BUY":
                sl = prev_m5_low - buffer
                # Check if SL is too close (safety) - min 500 points
                min_dist = 500 * mt5.symbol_info(symbol).point
