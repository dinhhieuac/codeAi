    ...
    try:
        ...
        if positions:
            for pos in positions:
                ...
                # Check if price is close (within 50 points for BTCUSD)
                if abs(pos.price_open - signal_price) > 50:
                    continue
        ...
        for deal in deals:
            ...
            # Check if price is close (within 50 points for BTCUSD)
            if abs(deal.price - signal_price) > 50:
                continue
...
def analyze_signal(signal_id):
    ...
    if result == 'win':
        ...
        if order_info.get('close_price') and signal_dict['sl']:
            ...
            if sl_distance < 1:  # Hit SL (for BTCUSD)
                analysis['analysis'].append({
                    'type': 'error',
                    'title': '🛑 Hit Stop Loss',
                    'message': f"Price hit SL at {signal_dict['sl']:.2f}"
                })
                ...
                if signal_dict['sl'] and signal_dict['price']:
                    sl_distance_points = abs(signal_dict['price'] - signal_dict['sl'])
                    if sl_distance_points < 100:
                        analysis['recommendations'].append({
                            'type': 'warning',
                            'title': '⚠️ SL Too Tight',
                            'message': f"SL distance: ~{sl_distance_points:.1f} points. Consider widening SL to at least 100-200 points to avoid noise."
                        })
                    elif sl_distance_points > 500:
                        analysis['recommendations'].append({
                            'type': 'info',
                            'title': '💡 SL Too Wide',
                            'message': f"SL distance: ~{sl_distance_points:.1f} points. Consider tightening SL to reduce risk per trade."
                        })
            
            elif tp_distance < 1:  # Hit TP (shouldn't happen for loss)
                pass
            else:
                analysis['analysis'].append({
                    'type': 'warning',
                    'title': '👤 Manual Close',
                    'message': f"Trade closed manually at {order_info.get('close_price', 0):.2f}"
                })
...
if __name__ == '__main__':
    print(f"🚀 BTCUSD Dashboard running on http://127.0.0.1:5001")
