...
from config_gbpusd import *
from risk_manager import GBPUSD_RiskManager
...
# Setup logging
log_file = os.path.join('logs', 'gbpusd_bot.log') if os.path.exists('logs') else 'gbpusd_bot.log'
os.makedirs('logs', exist_ok=True)
log_file = os.path.join('logs', 'gbpusd_bot.log')
...
class GBPUSD_Bot:
    def __init__(self):
        ...
        self.risk_manager = GBPUSD_RiskManager()
    ...
    def setup_mt5(self):
        logging.info("=" * 60)
        logging.info("🚀 KHỞI TẠO BOT GBPUSD")
        logging.info("=" * 60)
        
        # Log config đang sử dụng
        try:
            from config_gbpusd import selected_config, CONFIG_INDEX
    ...
    def calculate_position_size(self, stop_loss_pips):
        ...
        # 1 pip GBPUSD = $10 cho 1 lot (1 lot = 100,000 units, 1 pip = 0.0001)
        # Ví dụ: Giá tăng từ 1.2500 → 1.2501 (1 pip) với 1 lot → Profit = 100,000 × 0.0001 = $10.00
    ...
    def execute_trade(self, signal_type, sl_pips, tp_pips, signal_strength=0):
        ...
        # Tính SL theo USD: 1 pip GBPUSD = $10 cho 1 lot (1 lot = 100,000 units, 1 pip = 0.0001)
        # Ví dụ: Giá tăng từ 1.2500 → 1.2501 (1 pip) với 1 lot → Profit = 100,000 × 0.0001 = $10.00
        ...
        request_base = {
            ...
            "magic": 202411,
            "comment": f"GBPUSD_Bot_{signal_type}",
    ...
    def run_bot(self):
        ...
        logging.info("=" * 60)
        logging.info("🚀 BOT GBPUSD BẮT ĐẦU CHẠY")
        logging.info("=" * 60)
                ...
                if signal:
                    ...
                    if action != 'HOLD':
                            ...
                            if enable_tp_boost and 'rsi' in df.columns:
                                ...
                                if action == "BUY" and rsi_current > rsi_threshold_up:
                                    ...
                                    if self.use_telegram and should_send_signal:
                                        ...
                                        pip_value_per_lot = 10  # GBPUSD: 1 pip = $10 cho 1 lot
                        ...
                        if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                            ticket = result.order
                            logging.info("=" * 60)
                            logging.info(f"✅ LỆNH  {action} GBPUSD THÀNH CÔNG!")
                            logging.info("=" * 60)
                            ...
                            if self.use_telegram:
                                success_message = (
                                    f"✅ <b>LỆNH {action} GBPUSD THÀNH CÔNG</b>\n\n"
    ...
    def _manage_trailing_stops(self):
        ...
        if df is None or len(df) < 14:
            ...
            if atr_value is not None:
                atr_value = atr_value / 0.0001  # Convert to pips (GBPUSD: 1 pip = 0.0001)
...
def main():
    logging.info("=" * 60)
    logging.info("🚀 KHỞI ĐỘNG BOT GBPUSD")
    logging.info("=" * 60)
