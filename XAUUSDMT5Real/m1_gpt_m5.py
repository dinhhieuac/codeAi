...
def check_momentum_candle(df_m1, direction):
    ...
    # Tính kích thước nến (pips)
    candle_size_pips = abs(close_price - open_price) / point / 10
    
    if direction == 'BUY':
        # BUY: Kiểm tra nến tăng mạnh
        if close_price > open_price and candle_size_pips > MOMENTUM_CANDLE_MAX_PIPS:
            return True, f"Nến tăng quá mạnh ({candle_size_pips:.1f} pips > {MOMENTUM_CANDLE_MAX_PIPS} pips) - Chờ pullback"
    
    elif direction == 'SELL':
        # SELL: Kiểm tra nến giảm mạnh
        if close_price < open_price and candle_size_pips > MOMENTUM_CANDLE_MAX_PIPS:
            return True, f"Nến giảm quá mạnh ({candle_size_pips:.1f} pips > {MOMENTUM_CANDLE_MAX_PIPS} pips) - Chờ pullback"
...
def send_order(trade_type, volume, df_m1=None, m5_trend=None, m1_signal=None, signal_type=None, adx_m5_current=None, atr_pips=None, spread_points=None, deviation=20):
    """
...
def run_bot():
    ...
    while True:
        ...
        print(f"\n  ┌─ [BƯỚC 1] Kiểm tra xu hướng M5 (EMA50)")
        ...
        if open_positions <1:
            ...
            if not filters_passed:
                ...
                if not cooldown_allowed:
                    ...
                    send_order(mt5.ORDER_TYPE_BUY, VOLUME, df_m1, m5_trend=m5_trend, m1_signal=m1_signal, signal_type=signal_type, adx_m5_current=adx_m5_current, atr_pips=atr_pips, spread_points=spread_points)
                ...
                if not cooldown_allowed:
                    ...
                    send_order(mt5.ORDER_TYPE_SELL, VOLUME, df_m1, m5_trend=m5_trend, m1_signal=m1_signal, signal_type=signal_type, adx_m5_current=adx_m5_current, atr_pips=atr_pips, spread_points=spread_points)
