...
# ============================================================================
# CHIẾN THUẬT PULLBACK KẾT HỢP EMA & RSI (KHUNG M15)
# ============================================================================

# Bật/tắt chiến thuật Pullback
# True: Sử dụng chiến thuật Pullback (giá pullback về EMA, RSI xác nhận, Engulfing)
# False: Sử dụng logic cũ (signal strength)
USE_PULLBACK_STRATEGY = True  # True: Bật Pullback, False: Tắt (dùng logic cũ)

# EMA periods cho chiến thuật Pullback (theo pullback.md)
EMA_FAST = 21   # EMA ngắn hạn (21)
EMA_MID = 50    # EMA trung bình (50)
EMA_SLOW = 200  # EMA dài hạn (200)

# Khoảng cách tối đa để coi là pullback về EMA (đơn vị: pips)
# Giá phải nằm trong khoảng EMA ± PULLBACK_TOLERANCE_PIPS để coi là pullback
PULLBACK_TOLERANCE_PIPS = 30  # pips (≈ $0.30 với 0.01 lot)

# RSI điều kiện cho Pullback (theo pullback.md)
# BUY: RSI < 30 (quá bán)
# SELL: RSI > 70 (quá mua)
PULLBACK_RSI_BUY_MAX = 30   # RSI tối đa cho BUY (quá bán)
PULLBACK_RSI_SELL_MIN = 70  # RSI tối thiểu cho SELL (quá mua)

# Bật/tắt yêu cầu nến Engulfing
# True: Bắt buộc có nến Engulfing (Bullish cho BUY, Bearish cho SELL)
# False: Không bắt buộc Engulfing
REQUIRE_ENGULFING = True  # True: Bắt buộc, False: Không bắt buộc

# Bật/tắt xác nhận MACD cho Pullback (theo best practices)
# True: Yêu cầu MACD histogram chuyển từ âm sang dương (BUY) hoặc từ dương sang âm (SELL)
# False: Không yêu cầu MACD confirmation
REQUIRE_MACD_CONFIRMATION = True  # True: Bắt buộc, False: Không bắt buộc

# Bật/tắt xác nhận Volume cho Pullback (theo best practices)
# True: Yêu cầu volume tăng khi giá bounce từ pullback
# False: Không yêu cầu volume confirmation
REQUIRE_VOLUME_CONFIRMATION = False  # True: Bắt buộc, False: Không bắt buộc (tắt mặc định vì không phải broker nào cũng có volume)

# Cải thiện SL placement theo pullback.md
# True: SL đặt dưới đáy pullback (BUY) hoặc trên đỉnh pullback (SELL)
# False: SL dựa trên ATR như cũ
USE_PULLBACK_SL = True  # True: SL theo pullback, False: SL theo ATR

# Option TP tại EMA kế tiếp (theo pullback.md)
# True: TP có thể đặt tại EMA kế tiếp (EMA200 cho BUY, EMA200 cho SELL)
# False: TP chỉ dựa trên ATR/R:R ratio
...
# Lưu ý: Nếu USE_PULLBACK_STRATEGY = True, MIN_SIGNAL_STRENGTH sẽ không được sử dụng
...
MAX_ATR = 700  # Đơn vị: pips (≈ $50 với 1 lot, tránh volatility cực đại cho vàng)
...
# ⚠️ Giảm từ 50 xuống 40 pips để tránh vào lệnh khi spread quá cao (chi phí cao)
MAX_SPREAD = 40  # Đơn vị: pips (giảm từ 50 xuống 40 để tăng chất lượng entry)
...
account_json_path = project_root / "XAUUSD_BOT_FULL_V2_DEEPSEEK/md5_accout.json"  # Lưu ý: file có tên "accout" (không phải "account")
