"""
Grid Step — Zone lock + Break/Hold/Continue (bot tách từ strategy_grid_step_5min).

Sau SL (lệnh đóng lỗ): không dùng cooldown phút cố định. Khóa vùng quanh giá thoát SL:
  lower_zone = mid − step, upper_zone = mid + step.
Không đặt lưới khi giá trong vùng này; chỉ vào lại khi cấu trúc Break→Hold→Continue
trên nến tổng hợp (mặc định 15s, cấu hình zone_bar_seconds).

LIMIT/STOP như bản 5min; volume cố định (không martingale). Tắt pause N phút sau thua liên tiếp
mặc định — thay bằng rule vùng (có thể bật lại trong JSON nếu cần).

parameters.sl_tp_offset_price (tùy chọn, giá): khoảng cách SL và TP so với entry **tách khỏi**
bước lưới (step). Mặc định / bỏ qua / ≤0 → dùng đúng step (hành vi cũ: SL=TP=±step).
Ví dụ step=5 nhưng XAU nhiễu: đặt 8–15 để giảm bị quét SL liên tục (mỗi lần thua sẽ lớn hơn).

parameters.trend_gate_* (tùy chọn): khi không có position, chỉ đặt cặp lưới nếu nến gần đây có
**dịch chuyển có hướng** (efficiency ratio), giảm vào lệnh trong vùng đi ngang ping-pong.
Chỉ áp dụng lúc flat (có position thì không chặn để quản lý lưới). Mặc định trend_gate_enabled=false.
...
COOLDOWN_FILE = os.path.join(SCRIPT_DIR, "grid_cooldown_zone_lock.json")
PAUSE_FILE = os.path.join(SCRIPT_DIR, "grid_pause_zone_lock.json")
ORDER_COMMENT_PREFIX = "GridZone"
# Pause chung mọi step: khi bật single_loss_reset — tránh step khác vẫn giao dịch.
SINGLE_LOSS_RESET_PAUSE_STRATEGY_KEY = "Grid_ZoneLock_single_loss_kill"
...
def check_single_loss_should_trigger(
    ...
    rows = get_last_n_closed_summaries_bot(
        symbol, magic, scan, days_back=1, comment_prefix=ORDER_COMMENT_PREFIX
    )
    try:
...
def sync_closed_orders_from_mt5(config, strategy_name=None):
    """Đồng bộ profit/close_time từ MT5 history (comment GridZone*)."""
    closed = get_closed_deals_bot(
        config["symbol"], config["magic"], days_back=1, comment_prefix=ORDER_COMMENT_PREFIX
    )
    if not closed:
        return 0
    sn = strategy_name if strategy_name is not None else config.get("strategy_name", "Grid_ZoneLock")
...
def check_grid_step_db():
    ...
    try:
        cur.execute("SELECT COUNT(*) FROM grid_pending_orders WHERE strategy_name = 'Grid_ZoneLock'")
        n = cur.fetchone()[0]
        cur.execute("""
            SELECT status, COUNT(*) as c FROM grid_pending_orders WHERE strategy_name = 'Grid_ZoneLock'
            GROUP BY status
        """)
        by_status = {row['status']: row['c'] for row in cur.fetchall()}
        print(f"\n[grid_pending_orders] Grid_ZoneLock: {n} dong")
        for st, c in by_status.items():
            print(f"   - {st}: {c}")
        cur.execute("""
            SELECT ticket, order_type, price, status, position_ticket, placed_at, filled_at
            FROM grid_pending_orders WHERE strategy_name = 'Grid_ZoneLock'
    ...
    # Bảng orders (Grid_ZoneLock)
    try:
        cur.execute("SELECT COUNT(*) FROM orders WHERE strategy_name = 'Grid_ZoneLock'")
        n_ord = cur.fetchone()[0]
        cur.execute("""
            SELECT ticket, order_type, volume, open_price, sl, tp, profit, open_time, comment
            FROM orders WHERE strategy_name = 'Grid_ZoneLock' ORDER BY open_time DESC LIMIT 5
        """)
        rows_ord = cur.fetchall()
        print(f"\n[orders] Grid_ZoneLock: {n_ord} dong")
        if rows_ord:
            for r in rows_ord:
...
def _comment_for_step(step):
    """Comment MT5: GridZone / GridZone_5 (khớp zone_lock FSM)."""
    return ORDER_COMMENT_PREFIX if step is None else f"{ORDER_COMMENT_PREFIX}_{step}"
...
def trend_gate_eval(
    ...
    min_net_move_abs: float,
):
    """
    Efficiency ratio ≈ Kaufman ER: |Δclose| / Σ|Δclose_i|.
    Thị trường đi ngang ping-pong: tổng đường đi dài nhưng đóng cửa gần nhau → ER thấp.
    Trả (True, detail) nếu cho phép đặt lưới; (False, lý do) nếu chặn.
    """
    tf = _TIMEFRAME_NAME_TO_MT5.get(str(tf_name).strip().upper(), mt5.TIMEFRAME_M5)
    n = max(int(lookback), 3)
    rates = mt5.copy_rates_from_pos(symbol, tf, 0, n + 1)
    if rates is None or len(rates) < n + 1:
        return False, "trend_gate:no_rates"
    ...
    if path <= 1e-12:
        return False, "trend_gate:flat_path"
    er = net / path
    if er < float(min_efficiency):
        return False, f"trend_gate:chop_er={er:.3f}<{min_efficiency}"
    if min_net_move_abs > 0 and net < float(min_net_move_abs):
        return False, f"trend_gate:weak_net={net:.3f}<{min_net_move_abs}"
    return True, f"trend_gate:ok_er={er:.3f}_net={net:.3f}"


def get_pending_orders(symbol, magic, step=None):
    """Lấy danh sách lệnh chờ (pending). step=None = tất cả; step=N chỉ lấy comment GridZone_N."""
...
def cancel_all_pending(symbol, magic, strategy_name="Grid_ZoneLock", account_id=0, step=None):
...
def sync_grid_pending_status(symbol, magic, strategy_name="Grid_ZoneLock", account_id=0, sl_tp_price=5.0, info=None, step=None):
    ...
    for row in db.get_grid_pending_by_status(strategy_name, symbol, "PENDING"):
        ...
        if pos:
            ...
            # SL/TP: entry ± sl_tp_price (thực tế là sl_tp_offset / step do caller truyền).
            off = float(sl_tp_price)
            entry = float(pos.price_open)
            if pos.type == mt5.ORDER_TYPE_BUY:
                sl, tp = round(entry - off, digits), round(entry + off, digits)
            else:
                sl, tp = round(entry + off, digits), round(entry - off, digits)
...
def ensure_position_sl_tp(symbol, magic, sl_tp_price, info, step=None):
    ...
    for pos in positions:
        ...
        if pos.type == mt5.ORDER_TYPE_BUY:
            want_sl = round(entry - off, info.digits)
            want_tp = round(entry + off, info.digits)
        else:
            want_sl = round(entry + off, info.digits)
            want_tp = round(entry - off, info.digits)
...
def strategy_grid_step_logic(config, error_count=0, step=None):
    """Chạy logic grid zone-lock. step=None → strategy_name Grid_ZoneLock; multi-step → Grid_ZoneLock_{step}."""
    symbol = config['symbol']
    volume = config['volume']
    magic = config['magic']
    params = config.get('parameters', {})
    # step=None: một kênh — strategy_name "Grid_ZoneLock"
    if step is None:
        ...
        sl_tp_price = step
        strategy_name = f"Grid_ZoneLock_{step}"
        step_filter = step
    try:
        sl_tp_dist = float(params.get("sl_tp_offset_price", 0) or 0)
    except (TypeError, ValueError):
        sl_tp_dist = 0.0
    if sl_tp_dist <= 0:
        sl_tp_dist = float(sl_tp_price)
    ...
    zone_lock_enabled = bool(params.get("zone_reentry_after_sl_enabled", True))
    zone_state_path = str(params.get("zone_lock_state_path") or "").strip() or os.path.join(
        SCRIPT_DIR, "grid_zone_lock_state.json"
    )
    try:
        zone_sl_deal_max_age = float(params.get("zone_sl_deal_max_age_seconds", 28800) or 28800)
    except (TypeError, ValueError):
        zone_sl_deal_max_age = 28800.0
    try:
        zone_buffer_cfg = float(params.get("zone_break_buffer_price", -1) or -1)
    except (TypeError, ValueError):
        zone_buffer_cfg = -1.0
    try:
    ...
    trend_gate_enabled = bool(params.get("trend_gate_enabled", False))
    trend_gate_timeframe = str(params.get("trend_gate_timeframe", "M5") or "M5").strip()
    try:
        trend_gate_lookback_bars = int(params.get("trend_gate_lookback_bars", 16) or 16)
    except (TypeError, ValueError):
        trend_gate_lookback_bars = 16
    try:
        trend_gate_min_efficiency = float(params.get("trend_gate_min_efficiency", 0.18) or 0.18)
    except (TypeError, ValueError):
        trend_gate_min_efficiency = 0.18
    try:
        trend_gate_min_net_move_factor = float(params.get("trend_gate_min_net_move_factor", 1.2) or 1.2)
    except (TypeError, ValueError):
        trend_gate_min_net_move_factor = 1.2
    ...
    sync_grid_pending_status(symbol, magic, strategy_name, config.get('account'), sl_tp_dist, info, step_filter)

    # Đặt lại SL/TP cho position nếu broker không kế thừa từ lệnh chờ
    ensure_position_sl_tp(symbol, magic, sl_tp_dist, info, step_filter)
    ...
    if single_loss_reset_enabled and single_loss_reset_pause_minutes > 0:
        ...
        if react and last_close_ts and last_close_dt is not None:
            if not is_paused(SINGLE_LOSS_RESET_PAUSE_STRATEGY_KEY, now_utc=mt5_now):
                ...
                print(
                    f"🛑 [single_loss_kill] 1 lệnh thua (MT5 history, {ORDER_COMMENT_PREFIX}*) "
                ...
                msg = (
                    ...
                    f"1 lệnh đóng thua gần nhất ({ORDER_COMMENT_PREFIX}*).\n"
    ...
    if consecutive_loss_pause_enabled and consecutive_loss_pause_minutes > 0 and consecutive_loss_count > 0:
        ...
        profits, last_close_time_str = get_last_n_closed_profits_bot(symbol, magic, consecutive_loss_count, days_back=1, comment_prefix=ORDER_COMMENT_PREFIX)
    ...
    # Zone lock + Break/Hold/Continue (sau basket TP / single_loss / consecutive; chặn đặt lưới mới)
    tick_zone = mt5.symbol_info_tick(symbol)
    if zone_lock_enabled and tick_zone:
        mid_px = (float(tick_zone.bid) + float(tick_zone.ask)) * 0.5
        buf = zone_buffer_cfg
        if buf < 0:
        ...
        allowed, zreason, just_locked = zone_lock_tick(
            ...
            grid_step=grid_step_price,
            zone_buffer=buf,
            bar_seconds=zone_bar_seconds,
            tick_mid=mid_px,
        ...
        if just_locked:
            cancel_all_pending(symbol, magic, strategy_name, config.get("account"), step_filter)
        if not allowed:
            _zk = (step_filter, zreason)
            if not hasattr(strategy_grid_step_logic, "_zl_log") or strategy_grid_step_logic._zl_log != _zk:
                print(f"⏸️ [zone_lock] step={step_filter} {zreason}")
                strategy_grid_step_logic._zl_log = _zk
            return error_count, 0
    ...
    #    - Có position (LIMIT grid): hủy pending còn lại, đặt cặp mới quanh anchor
    #    - Có position + STOP grid: pending_stop_float_gate_phase (PnL vs x / y) trong grid_zone_reentry_fsm
    ...
    if grid_pending_stop_enabled and positions:
        ...
        if phase == "weak":
            ...
            if not hasattr(strategy_grid_step_logic, "_ps_float_log") or strategy_grid_step_logic._ps_float_log != _wg:
                print(
                    f"⏸️ [pending_stop_float] step={step_filter} float={fpnl:.2f} < x={pending_stop_float_gate_x:.2f} "
        ...
        if phase == "neutral":
            ...
            if len(pendings) == 1:
                ...
                if not hasattr(strategy_grid_step_logic, "_ps_float_neutral_log") or strategy_grid_step_logic._ps_float_neutral_log != _wg:
                    print(
                        f"⏸️ [pending_stop_float] step={step_filter} float={fpnl:.2f} trong [x={pending_stop_float_gate_x:.2f}, "
    ...
    # Cooldown mức grid — tắt khi bật zone_lock (thay bằng rule vùng)
    cooldown_minutes = int(float(params.get('cooldown_minutes', 0) or 0))
    if zone_lock_enabled:
        cooldown_minutes = 0
    if cooldown_minutes > 0:
    ...
    if trend_gate_enabled and not positions and mt5.symbol_select(symbol, True):
        mn = (
            float(grid_step_price) * trend_gate_min_net_move_factor
            if trend_gate_min_net_move_factor > 0
            else 0.0
        )
        ok_tg, tg_reason = trend_gate_eval(
            symbol,
            trend_gate_timeframe,
            trend_gate_lookback_bars,
            trend_gate_min_efficiency,
        ...
        if not ok_tg:
            _tgk = (step_filter, tg_reason)
            if not hasattr(strategy_grid_step_logic, "_tg_log") or strategy_grid_step_logic._tg_log != _tgk:
                print(
                    f"⏸️ [{strategy_name}] {tg_reason} "
                    f"(tf={trend_gate_timeframe} lookback={trend_gate_lookback_bars})"
                )
                strategy_grid_step_logic._tg_log = _tgk
            return error_count, 0

    # SL/TP đối xứng: ±sl_tp_dist (mặc định = step; có thể tách bằng parameters.sl_tp_offset_price).
    off = float(sl_tp_dist)
    filling = mt5.ORDER_FILLING_IOC if (info.filling_mode & 2) else mt5.ORDER_FILLING_FOK
    comment = _comment_for_step(step_filter)
    sl_buy = buy_price - off
    tp_buy = buy_price + off
    sl_sell = sell_price + off
    tp_sell = sell_price - off
    sl_buy = round(sl_buy, info.digits)
    tp_buy = round(tp_buy, info.digits)
    sl_sell = round(sl_sell, info.digits)
    tp_sell = round(tp_sell, info.digits)
...
def _format_last_two_closed_for_heartbeat(symbol, magic):
    """Chuỗi ngắn: 2 position đóng gần nhất (GridZone*), profit + giờ đóng UTC."""
    rows = get_last_n_closed_summaries_bot(symbol, magic, 2, days_back=1, comment_prefix=ORDER_COMMENT_PREFIX)
    if not rows:
        return "closed: —"
    parts = []
...
if __name__ == "__main__":
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    config_path = os.path.join(script_dir, "configs", "config_grid_step_zone_lock.json")
    ...
    if config and connect_mt5(config):
        ...
        _zone_on = bool(params.get("zone_reentry_after_sl_enabled", True))
        print(
            f"✅ Grid Zone-Lock Bot - Started ({label}) | pending={_pend_mode} | zone_reentry={_zone_on} | "
            f"loop_interval={_loop_interval_log_repr(loop_sleep_s)} | loop_log={log_n_txt}"
        )
        loop_count = 0
        try:
            while True:
                # Đồng bộ profit/close_time vào DB mỗi vòng (không phụ thuộc consecutive_loss pause)
                if steps_list is not None:
                    for step_val in steps_list:
                        sync_closed_orders_from_mt5(config, strategy_name=f"Grid_ZoneLock_{step_val}")
                else:
                    sync_closed_orders_from_mt5(config, strategy_name="Grid_ZoneLock")
                if steps_list is not None:
                    for step_val in steps_list:
                ...
                if loop_log_every_n > 0 and loop_count % loop_log_every_n == 0:
                    ...
                    zone_path = str(params.get("zone_lock_state_path") or "").strip() or os.path.join(
                        script_dir, "grid_zone_lock_state.json"
                    )
                    tick_mid_hb = (float(tick.bid) + float(tick.ask)) * 0.5 if tick else None
                    if _zone_on:
                        if steps_list:
                            zline = " | ".join(
                                get_zone_lock_status_line(zone_path, sv, tick_mid_hb) for sv in steps_list
                            )
                        else:
                            zline = get_zone_lock_status_line(zone_path, None, tick_mid_hb)
                    else:
                        zline = "zone: (tắt)"
                    print(
                        f"🔄 Grid Zone-Lock | loop_interval={_loop_interval_log_repr(loop_sleep_s)} | "
                        f"Steps: {steps_info} | Positions: {len(pos)} | Pending: {len(ords)} | "
                        f"Spread: {spread:.2f} | {zline} | {last2} | {pend2} | Loop #{loop_count}"
                    )

                if consecutive_errors >= 5:
                    error_msg = get_mt5_error_message(last_error_code)
                    msg = f"⚠️ [Grid Zone-Lock] 5 lỗi liên tiếp. Last: {error_msg}. Tạm dừng 2 phút..."
                    print(msg)
                    send_telegram(msg, config.get('telegram_token'), config.get('telegram_chat_id'))
                    time.sleep(120)
                    consecutive_errors = 0
            ...
            print("🛑 Grid Zone-Lock Bot Stopped")
            mt5.shutdown()
