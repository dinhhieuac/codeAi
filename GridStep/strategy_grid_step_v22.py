...
def check_chop_and_pause(strategy_name, account_id, step_val, chop_window_trades, chop_loss_count,
    ...
    """
    Phát hiện chop: trong N lệnh đóng gần nhất, >= chop_loss_count lệnh lỗ và entry nằm trong band hẹp (chop_band_steps * step).
    Trả về (True, last_close_time) nếu nên pause; (False, None) nếu không.
    """
...
def strategy_grid_step_logic(config, error_count=0, step=None):
    ...
    if consecutive_loss_pause_enabled or chop_pause_enabled:
        ...
        if is_paused(strategy_name, now_utc=mt5_now):
            ...
            return error_count, 0
        # V22: Chop Pause — phát hiện mẫu chop (nhiều lỗ trong vùng giá hẹp) thì pause riêng step
