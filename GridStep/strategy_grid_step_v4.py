"""
Grid Step Trading Bot v4 – Clone strategy_grid_step.py + Anti-Whipsaw (theo document/Anti-Whipsaw.md).
- Config riêng: configs/config_grid_step_v4.json
- DB riêng: trades_v4.db (phân biệt với bot Grid Step khác)
- Cooldown/Pause riêng: grid_cooldown_v4.json, grid_pause_v4.json
"""
...
DB_PATH_V4 = os.path.join(SCRIPT_DIR, "trades_v4.db")
db = Database(db_path=DB_PATH_V4)
COOLDOWN_FILE = os.path.join(SCRIPT_DIR, "grid_cooldown_v4.json")
PAUSE_FILE = os.path.join(SCRIPT_DIR, "grid_pause_v4.json")
...
def compute_whipsaw_score(step, side, level, now, closed_orders, weights=None):
    ...
    zone_60 = [o for o in zone if (now - o["open_time"]) <= timedelta(minutes=60)]
    ...
    score = (
        w_rsl * R_same30 + w_same_loss * L_same120 - w_same_win * W_same120
        + w_zone_loss * L_zone60 - w_zone_win * W_zone60 + w_c2 * C2
    )
...
# ---------- Cooldown / Pause (giống strategy_grid_step.py) ----------
...
def check_grid_step_v4_db():
    """Kiểm tra DB Grid Step v4. Chạy: python strategy_grid_step_v4.py --check-db"""
    path = db.db_path
    print(f"\n{'='*60}")
    print(f"[DB v4] path: {path}")
    ...
    cur = conn.cursor()
    try:
        cur.execute("SELECT COUNT(*) FROM grid_pending_orders")
        n = cur.fetchone()[0]
        cur.execute("SELECT status, COUNT(*) as c FROM grid_pending_orders GROUP BY status")
        by_status = {row['status']: row['c'] for row in cur.fetchall()}
        print(f"\n[grid_pending_orders] {n} dong")
    ...
    for row in db.get_grid_pending_by_status(strategy_name, symbol, "PENDING"):
        ...
        if pos:
            ...
            if pos.type == mt5.ORDER_TYPE_BUY:
                sl, tp = round(entry - step_sl, digits), round(entry + step_sl, digits)
            else:
                sl, tp = round(entry + step_sl, digits), round(entry - step_sl, digits)
            ...
            print(f"✅ Grid v4 lệnh khớp: ticket={ticket} → position={pos.ticket} ({order_type} @ {price})")
        else:
...
def strategy_grid_step_logic(config, error_count=0, step=None):
    ...
    if profit >= target_profit and positions:
        ...
        print(f"✅ [BASKET TP v4] step={_step_label} Profit {profit:.2f} >= {target_profit}, đóng tất cả.")
        close_all_positions(symbol, magic, step_filter)
        cancel_all_pending(symbol, magic, strategy_name, config.get('account'), step_filter)
        msg = f"✅ Grid Step v4 {step}: Basket TP hit. Profit={profit:.2f} | Closed all."
    ...
    if consecutive_loss_pause_enabled and consecutive_loss_pause_minutes > 0 and consecutive_loss_count > 0:
        ...
        if is_paused(strategy_name, now_utc=mt5_now):
            if not hasattr(strategy_grid_step_logic, "_last_pause_log_v4") or strategy_grid_step_logic._last_pause_log_v4 != strategy_name:
                print(f"⏸️ [{strategy_name}] Đang tạm dừng ({consecutive_loss_count} lệnh thua liên tiếp), chờ {consecutive_loss_pause_minutes} phút.")
                strategy_grid_step_logic._last_pause_log_v4 = strategy_name
    ...
    if spread_price > spread_max:
        if not hasattr(strategy_grid_step_logic, "_last_spread_log_v4") or strategy_grid_step_logic._last_spread_log_v4 != round(spread_price, 4):
            print(f"⏸️ v4 Spread={spread_price:.3f} > {spread_max} (bỏ qua)")
            strategy_grid_step_logic._last_spread_log_v4 = round(spread_price, 4)
        return error_count, 0
    if grid_step_price < spread_price:
        if not hasattr(strategy_grid_step_logic, "_last_grid_log_v4"):
            print(f"⚠️ Grid step (giá)={grid_step_price:.3f} quá nhỏ so với spread {spread_price:.3f}.")
            strategy_grid_step_logic._last_grid_log_v4 = True
        return error_count, 0
    ...
    if cooldown_minutes > 0:
        ...
        if is_level_in_cooldown(cooldown_levels, buy_price, cooldown_minutes, info.digits, step_filter) or is_level_in_cooldown(cooldown_levels, sell_price, cooldown_minutes, info.digits, step_filter):
            ...
            if not hasattr(strategy_grid_step_logic, "_last_cooldown_log_v4") or strategy_grid_step_logic._last_cooldown_log_v4 != _log_key:
                print(f"⏸️ Cooldown step={step_filter}: mức {buy_price}/{sell_price} trong {cooldown_minutes} phút, bỏ qua.")
                strategy_grid_step_logic._last_cooldown_log_v4 = _log_key
            return error_count, 0
    ...
    if whipsaw_enabled:
        ...
        if buy_comp:
            ...
            print(f"[WHIPSAW] BUY  level={buy_level} score={buy_score} = 3*{r}+3*{l120}-1*{w120}+1*{lz}-3*{wz}+3*{c2} | skip={skip_buy}")
        if sell_comp:
            ...
            print(f"[WHIPSAW] SELL level={sell_level} score={sell_score} = 3*{r}+3*{l120}-1*{w120}+1*{lz}-3*{wz}+3*{c2} | skip={skip_sell}")
        if whipsaw_dry_run:
    ...
    r1, r2 = None, None
    if not skip_buy:
        print(f"📤 [v4 step={_step_label}] BUY_STOP @ {buy_price} (SL={sl_buy}, TP={tp_buy})")
        r1 = place_pending(mt5.ORDER_TYPE_BUY_STOP, buy_price, sl_buy, tp_buy)
    if not skip_sell:
        print(f"📤 [v4 step={_step_label}] SELL_STOP @ {sell_price} (SL={sl_sell}, TP={tp_sell})")
    ...
    if not skip_buy and r1 and r1.retcode == mt5.TRADE_RETCODE_DONE:
        print(f"✅ Grid v4 step={_step_label}: BUY_STOP @ {buy_price:.2f} | ref={ref}")
        if cooldown_minutes > 0:
            save_cooldown_levels([buy_price], step_filter)
        db.log_grid_pending(r1.order, strategy_name, symbol, "BUY_STOP", buy_price, sl_buy, tp_buy, volume, config.get('account'))
    if not skip_sell and r2 and r2.retcode == mt5.TRADE_RETCODE_DONE:
        print(f"✅ Grid v4 step={_step_label}: SELL_STOP @ {sell_price:.2f} | ref={ref}")
        if cooldown_minutes > 0:
            save_cooldown_levels([sell_price], step_filter)
        db.log_grid_pending(r2.order, strategy_name, symbol, "SELL_STOP", sell_price, sl_sell, tp_sell, volume, config.get('account'))
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1].strip() == "--check-db":
        check_grid_step_v4_db()
        sys.exit(0)

    script_dir = os.path.dirname(os.path.abspath(__file__))
    config_path = os.path.join(script_dir, "configs", "config_grid_step_v4.json")
    ...
    if config and connect_mt5(config):
        ...
        print(f"✅ Grid Step v4 Bot - Started ({label}) | DB: {db.db_path} | Anti-Whipsaw: {'ON' if whipsaw_on else 'OFF'} (dry_run={dry})")
        loop_count = 0
        try:
            while True:
                ...
                if loop_count % 30 == 0:
                    ...
                    print(f"🔄 Grid Step v4 | Steps: {steps_info} | Positions: {len(pos)} | Pending: {len(ords)} | Spread: {spread:.2f} | Loop #{loop_count}")

                if consecutive_errors >= 5:
                    error_msg = get_mt5_error_message(last_error_code)
                    msg = f"⚠️ [Grid Step v4] 5 lỗi liên tiếp. Last: {error_msg}. Tạm dừng 2 phút..."
                    print(msg)
                    send_telegram(msg, config.get('telegram_token'), config.get('telegram_chat_id'))
                    time.sleep(120)
                    consecutive_errors = 0
            ...
            print("🛑 Grid Step v4 Bot Stopped")
            mt5.shutdown()
