...
from config_ethusd import *
from risk_manager import ETHUSD_RiskManager
...
# Setup logging
log_file = os.path.join('logs', 'ethusd_bot.log') if os.path.exists('logs') else 'ethusd_bot.log'
os.makedirs('logs', exist_ok=True)
log_file = os.path.join('logs', 'ethusd_bot.log')
...
class ETHUSD_Bot:
    def __init__(self):
        ...
        self.risk_manager = ETHUSD_RiskManager()
    ...
    def setup_mt5(self):
        logging.info("=" * 60)
        logging.info("🚀 KHỞI TẠO BOT ETHUSD")
        logging.info("=" * 60)
    ...
    def get_filling_mode(self, symbol):
        ...
        Args:
            symbol: Symbol cần kiểm tra (ví dụ: "ETHUSD")
    ...
    def calculate_position_size(self, stop_loss_pips):
        ...
        if not account_info:
            # Trả về MIN_LOT_SIZE từ config (0.1 cho ETHUSD)
            return MIN_LOT_SIZE if 'MIN_LOT_SIZE' in globals() else 0.1
            
        balance = account_info['balance']
        risk_amount = balance * (RISK_PER_TRADE / 100)
        
        # 1 pip ETHUSD = $1 cho 1 lot (1 lot = 1 ETH, 1 pip = 1 USD)
        # Ví dụ: Giá tăng từ 3000 → 3001 (1 pip) với 1 lot → Profit = 1 ETH × $1 = $1.00
        pip_value = 1  # $1 cho 1 lot
        position_size = risk_amount / (stop_loss_pips * pip_value)
        
        # Làm tròn và giới hạn kích thước
        # ⚠️ LƯU Ý: ETH trên MT5 có lot size tối thiểu là 0.1
        min_lot = MIN_LOT_SIZE if 'MIN_LOT_SIZE' in globals() else 0.1
        max_lot = MAX_LOT_SIZE if 'MAX_LOT_SIZE' in globals() else 1.0
    ...
    def execute_trade(self, signal_type, sl_pips, tp_pips, signal_strength=0):
        ...
        # ⚠️ LƯU Ý: ETH trên MT5 có lot size tối thiểu là 0.1
        lot_step = symbol_info.volume_step if symbol_info.volume_step and symbol_info.volume_step > 0 else 0.1
        min_lot_config = MIN_LOT_SIZE if 'MIN_LOT_SIZE' in globals() else 0.1
        ...
        # Tính SL theo USD: 1 pip ETHUSD = $1 cho 1 lot (1 lot = 1 ETH, 1 pip = 1 USD)
        # Ví dụ: Giá tăng từ 3000 → 3001 (1 pip) với 1 lot → Profit = 1 ETH × $1 = $1.00
        pip_value_per_lot = 1  # $1 cho 1 lot
        sl_usd = sl_pips * pip_value_per_lot * lot_size
        
        # Kiểm tra mode ATR SL/TP
        # Lấy từ config (đã được import từ config_ethusd.py)
        try:
        ...
        request_base = {
            ...
            "magic": 202411,
            "comment": f"ETHUSD_Bot_{signal_type}",
    ...
    def run_bot(self):
        ...
        logging.info("=" * 60)
        logging.info("🚀 BOT ETHUSD BẮT ĐẦU CHẠY")
        logging.info("=" * 60)
                ...
                if signal:
                    ...
                    if action != 'HOLD':
                        ...
                        if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                            ticket = result.order
                            logging.info("=" * 60)
                            logging.info(f"✅ LỆNH  {action} ETHUSD THÀNH CÔNG!")
                            logging.info("=" * 60)
                            ...
                            if self.use_telegram:
                                success_message = (
                                    f"✅ <b>LỆNH {action} ETHUSD THÀNH CÔNG</b>\n\n"
    ...
    def _manage_trailing_stops(self):
        ...
        stops_level_pips = stops_level if stops_level > 0 else 0  # ETHUSD: stops_level đã là pips
        ...
        for pos in positions:
            ...
            if pos.type == mt5.ORDER_TYPE_BUY:
                ...
                profit_pips = (current_price - entry_price)  # ETHUSD: 1 USD = 1 pip
            else:  # SELL
                current_price = tick.ask
                profit_pips = (entry_price - current_price)  # ETHUSD: 1 USD = 1 pip
            ...
            if enable_break_even and profit_pips >= break_even_start_pips and ticket not in self.breakeven_activated:
                ...
                # ⚠️ VỚI ETHUSD: 1 pip = 1 USD (không phải 0.01 như XAUUSD)
                if pos.type == mt5.ORDER_TYPE_BUY:
                    new_sl = entry_price + break_even_buffer_pips  # ETHUSD: buffer_pips đã là USD
            ...
            if ticket in self.breakeven_activated and atr_value is not None:
                ...
                if new_sl > current_sl and new_sl >= entry_price:
                    ...
                    if (new_sl < current_sl or current_sl == 0) and new_sl <= entry_price:
                        # Kiểm tra stops_level
                        if stops_level_pips > 0:
                            max_sl = current_price + stops_level_pips  # ETHUSD: stops_level_pips đã là USD
                            if new_sl > max_sl:
                                new_sl = max_sl
            ...
            if self.use_telegram and send_telegram:
                ...
                if tick and entry_price is not None:
                    ...
                    if pos_type == mt5.ORDER_TYPE_BUY:
                        current_price = tick.bid
                        profit_pips = (current_price - entry_price)  # ETHUSD: 1 USD = 1 pip
                        protected_pips = (new_sl - entry_price)  # ETHUSD: 1 USD = 1 pip
                    else:  # SELL
                        current_price = tick.ask
                        profit_pips = (entry_price - current_price)  # ETHUSD: 1 USD = 1 pip
                        protected_pips = (entry_price - new_sl)  # ETHUSD: 1 USD = 1 pip
    ...
    def _manage_partial_close(self, pos, profit_pips, ticket):
        ...
        min_lot = symbol_info.volume_min if symbol_info.volume_min > 0 else 0.1  # ETHUSD: min lot = 0.1
    ...
    def _close_partial_position(self, pos, close_volume, reason=""):
        ...
        # Tạo request để đóng một phần
            ...
            # Gửi Telegram notification
                ...
                # ⚠️ VỚI ETHUSD: 1 pip = 1 USD (không phải 0.01 như XAUUSD)
                if pos.type == mt5.ORDER_TYPE_BUY:
                    profit_pips = (close_price - pos.price_open)  # ETHUSD: 1 USD = 1 pip
                else:  # SELL
