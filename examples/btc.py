    ...
    def send_telegram_message(self, message: str) -> bool:
        ...
        try:
            ...
            if result.get('ok'):
                ...
                # Nếu lỗi do HTML parsing, thử lại không dùng HTML
                if 'HTML' in error_desc or 'parse' in error_desc.lower():
                    logger.warning(f"⚠️ Lỗi HTML parsing, thử lại không dùng HTML parse_mode")
                    payload_no_html = {
                        "chat_id": chat_id,
                        "text": message
                    }
                    response2 = requests.post(url, json=payload_no_html, timeout=10)
                    try:
                        result2 = response2.json()
                        if result2.get('ok'):
                            logger.info(f"✅ Đã gửi thành công (không dùng HTML parse_mode)")
                            return True
                        else:
                            logger.error(f"❌ Vẫn lỗi sau khi bỏ HTML: {result2.get('description', 'Unknown')}")
                    except:
                        logger.error(f"❌ Không thể parse response khi bỏ HTML: {response2.text[:500]}")
