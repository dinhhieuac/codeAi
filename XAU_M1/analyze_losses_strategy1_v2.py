...
def analyze_order_loss(ticket, order_type, open_time_str, open_price, sl, tp, close_price, profit):
    ...
    # M5 EMA200 (V2: Fixed - dùng EMA thực sự)
    ...
    df_m5_entry = calculate_adx(df_m5_entry, period=adx_period)
    adx_value = df_m5_entry.iloc[-1]['adx']
    ...
    df_m1_entry['atr'] = calculate_atr(df_m1_entry, period=atr_period)
    atr_val = df_m1_entry.iloc[-1]['atr']
    if pd.isna(atr_val) or atr_val <= 0:
        recent_range = df_m1_entry.iloc[-atr_period:]['high'].max() - df_m1_entry.iloc[-atr_period:]['low'].min()
    ...
    if pd.isna(adx_value) or adx_value < adx_min_threshold:
        issues.append(f"❌ ADX: {adx_value:.1f} < {adx_min_threshold} (cần >= {adx_min_threshold})")
    else:
        passed_conditions.append(f"✅ ADX: {adx_value:.1f} >= {adx_min_threshold}")
    ...
    # Check RSI (V2: Stricter thresholds)
    ...
    if order_type == "BUY":
        if last_ha['rsi'] <= rsi_buy_threshold:
            issues.append(f"❌ RSI: {last_ha['rsi']:.1f} <= {rsi_buy_threshold} (V2: cần > {rsi_buy_threshold})")
        else:
            passed_conditions.append(f"✅ RSI: {last_ha['rsi']:.1f} > {rsi_buy_threshold} (V2)")
    else:  # SELL
        if last_ha['rsi'] >= rsi_sell_threshold:
            issues.append(f"❌ RSI: {last_ha['rsi']:.1f} >= {rsi_sell_threshold} (V2: cần < {rsi_sell_threshold})")
        else:
            passed_conditions.append(f"✅ RSI: {last_ha['rsi']:.1f} < {rsi_sell_threshold} (V2)")
...
def generate_summary_report(all_stats, output_file):
    ...
    if issue_counts:
        for issue, count in sorted(issue_counts.items(), key=lambda x: x[1], reverse=True):
            ...
            issue_name = {
                ...
                'RSI': 'RSI không đạt ngưỡng (V2: BUY > 55, SELL < 45)'
            }.get(issue, issue)
    ...
    if issue_counts.get('M5_Trend', 0) > total_losses * 0.3:
        improvements.append({
            ...
            'suggestion': [
                'Kiểm tra lại EMA200 calculation (V2 đã fix)',
                'Thêm filter ADX để xác nhận trend strength (V2 đã có)',
    ...
    if issue_counts.get('ADX', 0) > total_losses * 0.3:
        improvements.append({
            ...
            'suggestion': [
                'V2 đã có ADX filter (>= 20)',
                'Có thể tăng threshold lên 25 nếu vẫn có nhiều lệnh thua',
    ...
    if issue_counts.get('Fresh_Breakout', 0) > total_losses * 0.3:
        improvements.append({
            ...
            'suggestion': [
                ...
                'V2 đã có confirmation check - có thể tăng thời gian chờ'
            ]
        })
    ...
    if issue_counts.get('RSI', 0) > total_losses * 0.3:
        improvements.append({
            ...
            'suggestion': [
                'V2 đã tăng ngưỡng RSI cho BUY (từ > 50 lên > 55)',
                'V2 đã giảm ngưỡng RSI cho SELL (từ < 50 xuống < 45)',
                'Có thể tăng thêm nếu vẫn có nhiều lệnh thua (BUY > 60, SELL < 40)',
    ...
    if exit_reasons.get('HIT_SL', 0) > total_losses * 0.5:
        improvements.append({
            ...
            'suggestion': [
                ...
                'V2 đã cải thiện SL buffer (từ 20 points lên 1.5x ATR)',
                'Có thể tăng buffer multiplier lên 2.0x ATR nếu vẫn quá chặt',
                'Thêm filter CHOP/RANGE để tránh trade trong market sideways (V2 đã có)',
    ...
    if all_conditions_passed > total_losses * 0.3:
        improvements.append({
            ...
            'suggestion': [
                'V2 đã có nhiều cải thiện: ADX, CHOP filter, RSI stricter, confirmation',
    ...
    improvements.append({
        ...
        'percentage': 0,
        'suggestion': [
            'V2 đã tăng spam filter (từ 60s lên 300s)',
            ...
            'V2 đã có ATR filter cho CHOP detection'
        ]
    })
    ...
    report.append("  1. V2 đã có nhiều cải thiện so với V1 (ADX, CHOP, RSI stricter, confirmation, SL buffer)")
    report.append("  2. So sánh kết quả V1 vs V2 để đánh giá hiệu quả")
    report.append("  3. Nếu vẫn có nhiều lệnh thua, xem xét các đề xuất trên")
    report.append("  4. Test trên demo account trước khi áp dụng thêm cải thiện")
