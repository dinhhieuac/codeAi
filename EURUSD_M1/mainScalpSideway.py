"""
Main file để chạy tất cả các bot Scalp Sideway
...
def main():
    # Get the directory where mainScalpSideway.py is located
    base_dir = os.path.dirname(os.path.abspath(__file__))
    
    # List of config files to run
    configs = [
        os.path.join(base_dir, "configs", "scalp_sideway_xau.json"),  # XAUUSD
        os.path.join(base_dir, "configs", "scalp_sideway_eur.json"),   # EURUSD
        os.path.join(base_dir, "configs", "scalp_sideway_btc.json"),   # BTCUSD
    ]
    
    # Bot script path
    bot_script = os.path.join(base_dir, "scalp_sideway.py")
    
    processes = []
    
    print("="*80)
    print("🚀 Starting Scalp Sideway Bots...")
    print("="*80)
