...
# Hệ số nhân ATR để tính SL và TP (theo grok.md: ATR Momentum Breakout Scalping)
# Theo grok.md: SL = Entry ± 1.5×ATR
ATR_MULTIPLIER_SL = 1.5  # Hệ số nhân ATR cho Stop Loss (theo grok.md: 1.5×ATR)
ATR_MULTIPLIER_TP = 3.5  # Hệ số nhân ATR cho Take Profit (tạm thời, sẽ dùng partial close strategy)
...
ENABLE_BREAK_EVEN = True  # True: Bật, False: Tắt
...
# Mốc TP1: Theo grok.md - Khi đạt +15 pips → Đóng 50% volume
PARTIAL_CLOSE_TP1_PIPS = 15  # Đơn vị: pips (theo grok.md: TP1 +15 pips)
PARTIAL_CLOSE_TP1_PERCENT = 50  # Đóng 50% volume (theo grok.md)

# Mốc TP2: Theo grok.md - Khi đạt +30 pips → Đóng 30% volume còn lại
PARTIAL_CLOSE_TP2_PIPS = 30  # Đơn vị: pips (theo grok.md: TP2 +30 pips)
PARTIAL_CLOSE_TP2_PERCENT = 30  # Đóng 30% volume còn lại (theo grok.md)

# Mốc TP3: Theo grok.md - Trailing stop cho phần còn lại
PARTIAL_CLOSE_TP3_PIPS = 50  # Đơn vị: pips (TP3 trailing, đặt giá trị cao để không trigger tự động)
PARTIAL_CLOSE_TP3_PERCENT = 0  # TP3 dùng trailing stop, không partial close (theo grok.md)
...
# - London/US: 15:00-02:00 +07 ≈ 03:00-14:00 EST (winter) hoặc 04:00-15:00 EDT (summer)
# - Asian session: 19:00-04:00 EST (next day) ≈ 20:00-05:00 EDT - TRÁNH GIAO DỊCH
# - NY Open: 08:00-10:00 EST/EDT - Tránh volatility cao khi mở cửa
NO_TRADE_SESSIONS = [
    ("08:00", "10:00"),  # NY Open - Giờ mở cửa thị trường New York (theo grok.md: tránh volatility cao)
    ("19:00", "04:00"),  # Asian session - Tránh giao dịch (theo grok.md: thấp volume, 19:00-04:00 EST)
    # Lưu ý: Session 19:00-04:00 sẽ wrap qua nửa đêm, cần xử lý đặc biệt trong risk_manager
]
...
account_json_path = project_root / "XAUUSD_BOT_FULL_V3_GROK_V2/md5_accout.json"  # Lưu ý: file có tên "accout" (không phải "account")
