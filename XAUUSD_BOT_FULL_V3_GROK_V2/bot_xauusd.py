...
class XAUUSD_Bot:
    ...
    def setup_mt5(self):
        # ⚠️ QUAN TRỌNG: Log này phải luôn hiển thị để xác nhận setup_mt5() được gọi
        logging.info("=" * 60)
        ...
        # ⚠️ QUAN TRỌNG: Phần này LUÔN được thực thi, không được skip
        logging.info("-" * 60)
        logging.info("⏰ QUY TẮC THỜI GIAN (từ time_check.py)")
        logging.info("-" * 60)
        logging.info("   🔍 Đang kiểm tra time_check module...")
    ...
    def run_bot(self):
        ...
        while True:
            try:
                ...
                # ====================================================================
                # RULE MỚI: Chỉ check tín hiệu khi nến M15 đóng (tránh fake breakout)
                # ====================================================================
                if len(df) < 2:
                    logging.debug("⚠️ Chưa đủ dữ liệu để kiểm tra nến đóng")
                    log_delay_and_sleep()
                    continue
                
                # Lấy thời gian nến cuối cùng (đã đóng)
                last_candle_time = pd.to_datetime(df.iloc[-1]['time'], unit='s')
                # Lấy thời gian nến trước đó để so sánh
                prev_candle_time = pd.to_datetime(df.iloc[-2]['time'], unit='s')
                
                # Tính thời gian hiện tại
                current_time = datetime.now()
                # Chuyển đổi last_candle_time sang datetime (nếu cần)
                if isinstance(last_candle_time, pd.Timestamp):
                    last_candle_time = last_candle_time.to_pydatetime()
                
                # Kiểm tra xem nến M15 hiện tại đã đóng chưa
                # Nến M15 đóng khi: thời gian hiện tại >= thời gian nến cuối + 15 phút
                candle_duration = timedelta(minutes=15)  # M15 = 15 phút
                next_candle_time = last_candle_time + candle_duration
                
                # Nếu nến chưa đóng (thời gian hiện tại < thời gian nến cuối + 15 phút)
                if current_time < next_candle_time:
                    time_until_close = (next_candle_time - current_time).total_seconds()
                    logging.debug(f"⏳ Nến M15 chưa đóng - Còn {int(time_until_close)} giây → Chờ nến đóng (Rule mới: chỉ check khi nến đóng)")
                    log_delay_and_sleep()
                    continue
                
                # Nến đã đóng → Kiểm tra xem có phải nến mới không (so với lần check trước)
                if not hasattr(self, 'last_checked_candle_time'):
                    self.last_checked_candle_time = None
                
                # Nếu đã check nến này rồi → Bỏ qua (tránh check lại cùng 1 nến)
                if self.last_checked_candle_time is not None and last_candle_time <= self.last_checked_candle_time:
                    logging.debug(f"⏸️ Đã check nến này rồi (Time: {last_candle_time.strftime('%Y-%m-%d %H:%M:%S')}) → Bỏ qua")
                    log_delay_and_sleep()
                    continue
                
                # Nến mới đã đóng → Cập nhật thời gian check và tiếp tục phân tích
                self.last_checked_candle_time = last_candle_time
                logging.info("=" * 60)
                logging.info(f"✅ NẾN M15 ĐÃ ĐÓNG - Bắt đầu phân tích tín hiệu")
                logging.info(f"   ⏰ Thời gian nến: {last_candle_time.strftime('%Y-%m-%d %H:%M:%S')}")
                logging.info("=" * 60)
                ...
                # Phân tích kỹ thuật (chỉ khi nến đã đóng)
                logging.info("🔍 Đang phân tích kỹ thuật (nến M15 đã đóng)...")
                # Truyền symbol để lấy H1 trend (Rule mới: H1 trend tăng → ưu tiên Buy, H1 trend giảm → ưu tiên Sell)
                        ...
                        if action != 'HOLD' and len(df) > 0:
                            ...
                            if enable_tp_boost and 'rsi' in df.columns:
                                ...
                                if action == "BUY" and rsi_current > rsi_threshold_up:
                                    ...
                                    # Lấy giá hiện tại
                                    tick = mt5.symbol_info_tick(self.symbol)
                                    current_price = tick.ask if tick else (df.iloc[-1]['close'] if len(df) > 0 else 0)
                                    ...
                                    if self.use_telegram and should_send_signal:
                                        ...
                                        logging.info(f"📈 TP BOOST KÍCH HOẠT - {self.symbol} {action}: Giá hiện tại {current_price:.2f}, TP gốc {tp_original_pips} pips (${tp_original_usd:.2f}) → TP sau boost {tp_pips} pips (${tp_boosted_usd:.2f}), SL {sl_pips:.0f} pips (${sl_usd:.2f}), Lot size: {estimated_lot_size:.2f}")
                                    ...
                                    # Lấy giá hiện tại
                                    tick = mt5.symbol_info_tick(self.symbol)
                                    current_price = tick.bid if tick else (df.iloc[-1]['close'] if len(df) > 0 else 0)
