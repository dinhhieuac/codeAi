...
def get_min_atr_threshold(symbol, config=None):
    ...
    if 'BTCUSDc' in symbol_upper or 'BTC' in symbol_upper:
        return 40.0
    ...
    for i in range(lookback, max_idx):
        ...
        if is_swing_high:
            ...
            if pd.notna(rsi_val) and rsi_val > min_rsi:
                # Lọc theo UpperWick: UpperWick < 1.3 ATR (loại bỏ swing high có wick quá lớn)
                candle = df_m1.iloc[i]
...
def find_swing_low_with_rsi(df_m1, lookback=5, min_rsi=30, max_idx=None):
    ...
    for i in range(lookback, max_idx):
        ...
        if is_swing_low:
            ...
            rsi_val = df_m1.iloc[i].get('rsi', None)
            if pd.notna(rsi_val) and rsi_val < min_rsi:
                # Lọc theo LowerWick: LowerWick < 1.3 ATR (loại bỏ swing low có wick quá lớn)
                candle = df_m1.iloc[i]
...
def m1_scalp_logic(config, error_count=0):
    ...
    try:
        ...
        if result.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            db.log_order(result.order, "M1_Scalp_BTCUSD", symbol, signal_type, volume, actual_entry, sl, tp, reason, account_id=config.get('account'))
