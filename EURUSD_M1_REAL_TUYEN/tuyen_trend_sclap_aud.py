...
def get_min_atr_threshold(symbol, config=None):
    ...
    if 'EURUSD' in symbol_upper or 'GBPUSD' in symbol_upper or 'USDJPY' in symbol_upper or 'AUDUSDc' in symbol_upper:
        return 0.00011
...
def get_pip_value_per_lot(symbol, symbol_info=None):
    ...
    if symbol_info:
        ...
        elif 'EURUSD' in symbol_upper or 'GBPUSD' in symbol_upper or 'AUDUSD' in symbol_upper:
            return 10.0
        else:
            return 10.0
...
def m1_scalp_logic(config, error_count=0):
    ...
    try:
        ...
        if result.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            db.log_order(result.order, "M1_Scalp_AUDUSD", symbol, signal_type, volume, current_price, sl, tp, reason, account_id=config.get('account'))
...
if __name__ == "__main__":
    ...
    config_path = os.path.join(script_dir, "configs", "config_tuyen_aud.json")
    config = load_config(config_path)
    
    consecutive_errors = 0
    if config and connect_mt5(config):
        print("\n" + "="*80)
        print(f"✅ M1 Scalp Bot - AUDUSD - Started")
        print(f"💱 Symbol: {config.get('symbol', 'N/A')}")
