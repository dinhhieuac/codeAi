...
def check_valid_pullback_buy(df_m1, swing_high_idx, max_candles=30, rsi_target_min=40, rsi_target_max=50, rsi_min_during_pullback=32, symbol=None):
    """
    ...
    # V2: Chỉ dùng nến đã đóng, không tính nến đang hình thành (tránh RSI/close thay đổi mỗi tick)
    pullback_start = swing_high_idx + 1
    last_completed_idx = len(df_m1) - 2 if len(df_m1) >= 2 else len(df_m1) - 1
    pullback_end = min(pullback_start + max_candles, last_completed_idx)
    ...
    # V2: Bắt buộc có symbol để kiểm tra slope, tránh chấp nhận pullback khi không validate được
    if symbol is None:
        return False, None, None, "Không thể kiểm tra slope (symbol không có)"
    symbol_info = mt5.symbol_info(symbol)
    if symbol_info:
...
def check_valid_pullback_sell(df_m1, swing_low_idx, max_candles=30, rsi_target_min=50, rsi_target_max=60, rsi_max_during_pullback=68, symbol=None):
    """
...
def m1_scalp_logic(config, error_count=0):
    """
    M1 Scalp Strategy Logic v2 - Swing High/Low + Pullback + Trendline Break
    V2 fixes: order_send None check, pullback chỉ nến đã đóng, slope bắt buộc khi có symbol.
    ...
    try:
        ...
        if buy_condition1:
            ...
            if len(swing_highs_with_rsi) == 0:
                ...
                pullback_valid, pullback_end_idx, pullback_candles, pullback_msg = check_valid_pullback_buy(
                    df_m1, swing_high_idx, max_candles=30, rsi_target_min=40, rsi_target_max=50, rsi_min_during_pullback=32, symbol=symbol
                )
            ...
            if sell_condition1:
                ...
                if len(swing_lows_with_rsi) == 0:
                    ...
                    pullback_valid, pullback_end_idx, pullback_candles, pullback_msg = check_valid_pullback_sell(
                        df_m1, swing_low_idx, max_candles=30, rsi_target_min=50, rsi_target_max=60, rsi_max_during_pullback=68, symbol=symbol
                    )
        ...
        # V2: Kiểm tra order_send trả về None (mất kết nối / lỗi MT5) trước khi dùng result
        if result is None:
            error_msg = "order_send() trả về None (mất kết nối hoặc lỗi MT5)"
            print(f"❌ {error_msg}")
        ...
        if result.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            db.log_order(result.order, config.get('strategy_name', 'M1_Scalp_XAUUSD_V2'), symbol, signal_type, volume, actual_entry, sl, tp, reason, account_id=config.get('account'))
...
if __name__ == "__main__":
    ...
    config_path = os.path.join(script_dir, "configs", "config_tuyen_xau_v2.json")
