...
def strategy_1_logic(config, error_count=0):
    ...
    if signal:
        ...
        # Log signal to DB
        try:
            db.log_signal(f"Strategy_1_Trend_HA_V2_{magic}", symbol, signal, price, sl, tp, 
        ...
        request = {
            ...
            "tp": tp,
            "magic": magic,
            "comment": str(config.get("order_comment") or f"S1_{config.get('magic')}"),
        ...
        if result.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            try:
                ...
                db.log_order(result.order, f"Strategy_1_Trend_HA_V2_{magic}", symbol, signal, volume, price, sl, tp, result.comment, account_id=config['account'], initial_sl=sl)
...
def run_multi_config():
    """
    Multi-Config Runner cho Strategy 1 Trend HA V2:
    - Chạy song song nhiều cấu hình thử nghiệm trên cùng 1 tài khoản Exness.
    - Mỗi cấu hình được định danh và cô lập hoàn toàn bằng Magic Number riêng biệt.
    - Hỗ trợ truyền danh sách config qua dòng lệnh: --configs config_1.json config_2.json ...
      hoặc tự động tải tất cả các file configs/config_exp_*.json.
    """
    import os
    import glob
    import argparse

    parser = argparse.ArgumentParser(description="Strategy 1 Trend HA V2 - Multi-Config Runner")
    parser.add_argument(
        "--configs",
        nargs="+",
        help="Danh sách các file config cần chạy song song (ví dụ: configs/config_exp_1.json configs/config_exp_2.json)"
    )
    parser.add_argument(
        "--scan-interval",
        type=float,
        default=1.0,
        help="Thời gian nghỉ (giây) giữa các chu kỳ quét (mặc định 1.0s)"
    )
    ...
    if args.configs:
        for c in args.configs:
            p = c if os.path.isabs(c) else os.path.join(configs_dir, c)
            if os.path.isfile(p):
                selected_files.append(p)
            elif os.path.isfile(os.path.join(script_dir, c)):
                selected_files.append(os.path.join(script_dir, c))
            else:
                print(f"⚠️ Không tìm thấy file config: {c}")
    else:
        # Tự động tìm các file config_exp_*.json
        exp_pattern = os.path.join(configs_dir, "config_exp_*.json")
        matched = sorted(glob.glob(exp_pattern))
        if matched:
            selected_files = matched
        else:
            # Fallback nếu chưa có file exp nào: dùng config_1_v2.json
            fallback = os.path.join(configs_dir, "config_1_v2.json")
            if os.path.isfile(fallback):
                selected_files = [fallback]

    if not selected_files:
        print("❌ Không tìm thấy file config hợp lệ nào để chạy!")
        return
    ...
    for path in selected_files:
        ...
        if not cfg:
            continue
        cfg_filename = os.path.basename(path)
        cfg["_config_file"] = cfg_filename
        magic = cfg.get("magic")
        if not magic:
            print(f"⚠️ Bỏ qua {cfg_filename}: Không có 'magic' number!")
            continue

        if magic in seen_magics:
            prev_file = seen_magics[magic]
            print(f"⚠️ CẢNH BÁO: Trùng Magic Number {magic} giữa [{cfg_filename}] và [{prev_file}]!")
            print(f"   Mỗi cấu hình thử nghiệm BẮT BUỘC phải dùng 1 Magic Number khác nhau để tránh quản lý nhầm lệnh.")
            continue
        seen_magics[magic] = cfg_filename

        instance_name = os.path.splitext(cfg_filename)[0]
    ...
    if not loaded_instances:
        print("❌ Không tải được cấu hình nào hợp lệ!")
        return

    # Kết nối MT5 với tài khoản của config đầu tiên
    primary_config = loaded_instances[0]["config"]
    if not connect_mt5(primary_config):
        print("❌ Kết nối MT5 thất bại!")
        return

    print("=" * 85)
    print("🚀 MULTI-CONFIG BOT: STRATEGY 1 TREND HA V2 (MULTI-EXPERIMENT)")
    print(f"👤 Tài khoản MT5: {primary_config.get('account')} | Server: {primary_config.get('server')}")
    print(f"📊 Số lượng Cấu hình đang chạy song song: {len(loaded_instances)}")
    print("=" * 85)
    for inst in loaded_instances:
        ...
        if len(desc) > 35:
            desc = desc[:32] + "..."
        p = c.get("parameters", {})
        rsi_buy = p.get("rsi_buy_threshold", 58)
        rsi_sell = p.get("rsi_sell_threshold", 42)
        adx_min = p.get("adx_min_threshold", 22)
        sl_m = p.get("sl_mode", "auto_m5")
        rr = p.get("reward_ratio", 1.5)
        print(f"  📌 [{inst['name']}] Magic: {c.get('magic')} | Vol: {c.get('volume')} | RSI: >{rsi_buy}/<{rsi_sell} | ADX>={adx_min} | SL: {sl_m} (RR {rr}) | {desc}")
    print("=" * 85)

    try:
        while True:
            # 1. Kiểm tra weekend flatten nếu được cấu hình
            if weekend_flatten_maybe(primary_config):
                print("🛑 [HA weekend] Hoàn tất flatten — dừng bot (mt5.shutdown).")
                break

            now_ts = time.time()

            # 2. Chạy logic độc lập cho từng config instance
            for inst in loaded_instances:
                cfg = inst["config"]
                name = inst["name"]

                if now_ts < inst["paused_until"]:
                    continue

                try:
                    c_errors, last_err = strategy_1_logic(cfg, inst["consecutive_errors"])
                    inst["consecutive_errors"] = c_errors

                    if c_errors >= 5:
                        err_msg = get_mt5_error_message(last_err)
                        w_msg = f"⚠️ [{name} - Magic {cfg.get('magic')}] 5 lần đặt lệnh thất bại liên tiếp ({err_msg}). Tạm dừng 2 phút..."
                        print(w_msg)
                        send_telegram(w_msg, cfg.get("telegram_token"), cfg.get("telegram_chat_id"))
                        inst["paused_until"] = now_ts + 120
                        inst["consecutive_errors"] = 0
                except Exception as e:
                    print(f"❌ Lỗi thực thi logic cho [{name}]: {e}")

            time.sleep(args.scan_interval)

    except KeyboardInterrupt:
        print("\n🛑 Multi-Config Bot đã dừng bởi người dùng.")
    finally:
        mt5.shutdown()
