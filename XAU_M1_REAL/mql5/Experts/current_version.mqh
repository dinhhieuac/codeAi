                       ...
                       S1TrailConfig &cfg)
{
   ...
   if(cfg.breakeven_enabled)
   {
      ...
      if(profit_points > breakeven_trigger_points)
      {
         ...
         const double sl_norm = NormalizeDouble(sl, digits);
   ...
   if(cfg.trailing_enabled && !modify)
   {
      ...
      if(profit_points > trailing_trigger_points)
      {
         ...
         if(pos_type == POSITION_TYPE_BUY)
         {
            const double candidate = NormalizeDouble(current_price - trail_dist, digits);
            if(candidate - sl_norm >= point)
            {
               new_sl = candidate;
               modify = true;
            }
         ...
         {
            const double candidate = NormalizeDouble(current_price + trail_dist, digits);
            if(sl == 0.0 || (sl_norm - candidate >= point))
            {
               new_sl = candidate;
               modify = true;
            }
