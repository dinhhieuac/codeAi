...
class BTCUSD_Bot:
    ...
    def calculate_position_size(self, stop_loss_pips):
        ...
        if not account_info:
            # Trả về MIN_LOT_SIZE từ config (0.01 cho BTCUSD)
            return MIN_LOT_SIZE if 'MIN_LOT_SIZE' in globals() else 0.01
            
        balance = account_info['balance']
        risk_amount = balance * (RISK_PER_TRADE / 100)
        
        # 1 pip BTCUSD = $1 cho 1 lot (1 lot = 1 BTC, 1 pip = 1 USD)
        # Ví dụ: Giá tăng từ 90000 → 90001 (1 pip) với 1 lot → Profit = 1 BTC × $1 = $1.00
        pip_value = 1  # $1 cho 1 lot
        position_size = risk_amount / (stop_loss_pips * pip_value)
        
        # Làm tròn và giới hạn kích thước
        # ⚠️ LƯU Ý: BTC trên MT5 có lot size tối thiểu là 0.01
        min_lot = MIN_LOT_SIZE if 'MIN_LOT_SIZE' in globals() else 0.01
    ...
    def execute_trade(self, signal_type, sl_pips, tp_pips, signal_strength=0):
        ...
        if atr_sl_tp_mode == "ATR_BOUNDED":
            ...
            # ⚠️ VỚI BTCUSD: 1 pip = 1 USD, broker_stops_level thường là points (0.01)
            ...
            if broker_stops_level > 0:
                # Convert points to pips: Với BTCUSD, 1 pip = 1 USD = 100 points (nếu point = 0.01)
